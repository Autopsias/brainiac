...
metadata:
  ...
  # under every existing constraint unchanged (the run's cap shared, overlay-style DRAFT+CHECK,
...
> (`overlay/keywords/`), writing voice (the workspace `overlay-style` skill) — is read
...
0. **Overlay load (every run — the personalization slot):**
   ...
   - **Degradation (mirrors the overlay-style kernel): a missing category, a missing `overlay/`, or a template-only scaffold ⇒ run neutral for that category and say so in the brief footer** — *"No overlay/<cat>/ found — running neutral; fill `<vault>/overlay/` to personalize (overlay/README.md)."* Never block, never invent.
...
- creating reply drafts (one cap per run; 30 for the unattended nightly,
  owner ruling 2026-09-04) — **governed by `overlay/cos/drafts.md`
...
| In-thread reply draft (`createReply`/`createReplyAll`, unsent, one cap per run) | draft-first | — | the DECISION (send or discard) is the owner's; discard is their one click — this run never deletes (delete is a Layer-2 deny) | drafts ledger · Drafts verification (E3) |
...
1½. **Lane-portable evidence access (v5.36, ING-05 — the same direction
   ...
   - **Unreadable ⇒ HELD, never silent.** An in-scope thread whose body the
     ...
     **(v5.55) `target-identity-unconfirmed`** is the deep-link primitive's —
     ...
     is declared — and, v7.2/DORM-01, no longer offered to the judge at all,
     because being offered is how it fired 38 times on run 178.**) A
   ...
   **v5.37 additions, as amended by v5.39:**
   ...
   - **(v5.42, EXT-06) `over-candidate-cap` AND THE READ-ORDER TIE-BREAK ARE
     ...
     - **While NO cap is declared, an `over-candidate-cap` row is a FAIL**
       ...
       **(v7.2, DORM-01) AND THE HOST NOW CHECKS IT
       (`cos_runverify.check_ledger_vocabulary`), BECAUSE THE ACCIDENT
       HAPPENED.** This clause said FAIL from v5.42 and nothing scored it, so
       run 178 held **38 of 218 rows** under the cap that does not exist. The
       cause was upstream of the run: `_HELD_REASONS` is printed VERBATIM into
       the judge's closed vocabulary, so the model was OFFERED the dormant
       word — it is now in `_DORMANT_HELD_REASONS`, which the prompt subtracts
       and the validator does not, so historical ledgers still validate. A
       pre-v7.2 ledger DEGRADES rather than retro-failing; failing a run for
       choosing a word we handed it is the instrument-fails-a-correct-run
       shape E29 refuses. **Re-introducing a cap needs a real PRODUCER, and
       there is none:** `cos_driver_accounting` writes a per-row `staging_cap`
       that copies `BODY_OPEN_CAP` — the READ cap — and no consumer has ever
       read the field. Declare the cap where the host can recount it, or
       `uncapped` remains the only honest state.
...
- Voice: the owner's own self-contained voice skill if one is installed, else the workspace **`overlay-style` skill** (DRAFT + CHECK modes) reading `overlay/voice/`; neutral register if neither. The kernel skill was renamed from `voice` to `overlay-style` on 2026-08-31 so a request naming "voice" can never cross-fire between the two.
