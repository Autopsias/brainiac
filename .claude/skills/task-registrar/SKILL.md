...
`docs/cutover/persistence-budget.md` LOCKS the OS-scheduled-task count at
...
unload-then-load of the `com.profile-a-brain.daily-brief` LaunchAgent; on
...
(`docs/cutover/export-egress-gate.md`) Leg 2 (classification ceiling) by
...
`docs/cutover/task-disposition.md`'s taxonomy), re-validate the budget check
...
- `docs/cutover/persistence-budget.md` — THE LOCK (1 host, 0 VM) this skill enforces
- `docs/cutover/task-disposition.md` — per-task disposition taxonomy (FOLD /
  DISTINCT-CADENCE / ON-INVOKE / OVERLAY-ONLY) the manifest rows are built from
- `docs/cutover/export-egress-gate.md` — PF-02, the gate every Cowork-bound
