...
4. **Score** S/X/T/MH (where applicable)/CAL per question against what the
   vault actually says, not against what `search` ranked. When a hit is
   doubtful, re-read the note itself:
   - **HOST LANE.** `brain get <id> --json`, or the file under `vault/brain/`
     or `vault/raw/` if you need the raw bytes.
   - **COWORK LANE.** The desk's `get` tool (`id: "<id>"`) or its `grep` tool.
     Never open the file: there is no `vault/` to open in a Cowork session, and
     going looking for one is the bypass this substrate exists to close.
