...
│                           worked example (PER-01); the active overlay lives
...
**The overlay is the only place owner identity lives.** `vault/brain/` and
...
there instead. `brain init --validate-overlay` (PER-02, minimal slice — full
...
classification: Internal         # Public|Internal|Confidential|Restricted|MNPI
created: 2026-06-27
updated: 2026-06-27
...
> stage before stdout** (unlabelled ⇒ MNPI ⇒ withheld; elevate with
...
>
> **Where the kernel skills live per client (SKL-02/03/04, s08):** the eight
> kernel/extras skills (`kb-curator`, `promote`, `vault-ingestion`,
> `vault-eval`, `save-conversation`, `curation`, `improve`, `task-registrar`)
> ship three ways from ONE canonical copy at `.claude/skills/<name>/SKILL.md`
...
> versioned marketplace (`.claude-plugin/marketplace.json` — `profile-a-kernel`
> + optional `profile-a-extras` plugins) is registered via
> `.claude/settings.json` `extraKnownMarketplaces` for the one-command-away
> install path (`/plugin marketplace add Autopsias/profile-a-brain` once,
> `/plugin marketplace update` to sync). **Codex** auto-loads the mirrored
...
- **Classification gate, default-deny.** `search/get/recent` filter by
  ...
  treated as the most-restrictive tier (MNPI) and is NOT surfaced** without an
