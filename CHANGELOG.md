...
- **A script emitting `type: proposal` daily was counted as 22 sources
  awaiting notes.** ENF-06 decides "operational" by `type:`, and deliberately
  keeps `proposal`/`report`/`analysis`/`review` out of that set because what
  decides is the subject, not the author. That reasoning is right and is not
  weakened. The gap it left is narrower: when `_log_rotation.py` emits
  `type: proposal` once a day, type is the only signal and it says "keep". The
  new `emitter_output` exclusion is ORTHOGONAL rather than a wider type list —
  the file's own frontmatter must SAY a program produced it (`proposed_by:`/
  `generated_by:` whose value IS a program path, or `provenance:` literally
  beginning `Generated by <program>`). It is counted under its own reason, and
  `brain health-report` now renders the unlinked-sources exclusions per reason
  instead of one aggregate, which is what makes a newly added exclusion class
  visible at all. On the reference vault: `unlinked_sources` 98 → 73, 25
  excluded. **The anchoring is load-bearing and measurement is what found it:**
  a looser rule matching any of those keys that merely MENTIONS a `.py` caught
  27, and two of the extra were a `type: decision-draft` and a
  `type: skill-patch` whose prose provenance names the scanner that informed
  them. Both are pinned as known negatives; loosening the pattern fails them.
- **The parallel release suite never ran in the release.** `ae6efb1` gave the
  pipeline AGENTS.md §8's parallel invocation (`-n 8 --dist loadfile --timeout
  300`), measured against the repo's own `.venv`. But the pipeline runs under
  whichever interpreter has pytest+build+twine — on this host
  `~/.brainiac/venv`, which carries bare pytest — so the phase aborted in three
  seconds with `unrecognized arguments: -n --dist --timeout`. It had been
  broken in the SHIPPING context since it landed, because no release ran in
  between to exercise it. The flags are now probed for and the phase degrades
  to sequential when the plugins are absent, saying so in its summary and in
  the evidence transcript — a missing test accelerator must not block a
  release, but running slow in silence is a regression wearing a pass.
- **A suite that never started reported a blank error.** pytest writes startup
  failures (unrecognised arguments, collection errors, missing plugins) to
  stderr, and `pytest_failure_summary` read only stdout — so the phase printed
  `test suite failed in the tag worktree:` with nothing after the colon. It now
  reads both streams and never returns an empty summary.

### Changed
- **The release pipeline no longer publishes npm — the tag push does, and it
  publishes better.** `npm-publish.yml` has published `brainiac-install` from
  CI over OIDC trusted publishing since v0.20.11: ~25 seconds, no credential,
  SLSA provenance attached. But the pipeline's own npm phase ran BEFORE the
  tag push, so it always won the race and the workflow found nothing to do.
  That cost an npm session token that expires every two hours (it broke the
  v0.20.22 run and cost a resume), and it shipped three releases with the
  provenance stripped — `npm view brainiac-install@<v> _npmUser.name` reads
  `GitHub Actions` for 0.20.11 and `autopsias` for 0.20.19/0.20.21/0.20.22,
  with no attestation on any of the three. GitHub also removes direct publish
  from 2FA-bypass tokens in January 2027, so the hand path had a deadline.
  The phase is gone; what it did that was actually verification is kept
  (`npm_pack_smoke` packs the exact tarball and runs it BEFORE the push, since
  an npm version is permanent), the public-git gate now states that the push
  publishes npm, and `phase_post_verify` waits for the registry and fails
  naming the workflow re-fire command. Phase numbers are derived from `PHASES`
  now instead of hand-written, which is what would otherwise have silently
  misnumbered the six phases after the one removed.
- **`install.sh` / `install.ps1` install on a machine with no Python.** Both
  hard-failed with "install Python 3.9+ first", even on the default path,
  where Python is only needed for the `pip --user` last resort and the `--dev`
  editable venv — uv and pipx each manage their own interpreter. `npx
  brainiac-install`, the path that looked like it covered that case, only
  printed the same advice. uv closes it properly: a self-contained binary that
  needs no Python and can fetch a managed CPython. With neither a usable
  `python3` nor uv present, the installers now fetch uv (visibly, saying so
  first) and install through it with `--python 3.12`. `--no-uv-bootstrap` /
  `-NoUvBootstrap` forbids the download and names both fixes instead. A
  machine that already has a good interpreter is unaffected — `--python` is
  passed only on the bootstrap path.
- **The install docs stopped telling people to install Python first.** The
  README's by-hand section led with `npx brainiac-install`, which needs Node
  **and** a Python toolchain, and framed the shell script as the "no `uv`?"
  fallback; it is now the path that needs nothing, and the npx caveat is
  stated. `docs/install/LLM-INSTALL.md` — the page an assistant is told to
  follow verbatim — explicitly says the stale advice is stale, so an agent
  reading it does not re-issue it. The npm package's own README says plainly
  that it cannot help a machine without Python and points at the shell
  installer. `publish-release` (the skill that drives a release) dropped from
  five gated acts to four, lost its `npm whoami` preflight and its npm-2FA
  relay instructions, and now requires the decision card to state that the
  `public-git` push is what publishes npm.
