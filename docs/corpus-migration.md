...
1. **Rule-based first pass** (deterministic, auditable):
   - git-crypt-designated zones (`10 People/`, `20 Companies/`, `70 Decisions/`,
     `30 Projects/Atlas.md`, transcripts, voice) → `Confidential` or higher.
   - Notes mentioning Atlas counterparties / deal terms / financials →
     `Restricted`; flagged-regime Secret/PII → `Secret`.
...
3. **Default-deny safety net:** anything the passes can't classify stays
   **unlabelled ⇒ Secret-at-gate** — invisible, not leaked. The migration is not
   "done" until the unlabelled count is driven to zero (or explicitly accepted).
4. **Human review** of every `Restricted`/`Secret` assignment and a sample of the
   rest.
