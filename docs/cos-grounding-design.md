...
| 7b | `$EV/interview/prompt.txt` (the phrasing leg's stdin, host-authored: note excerpts) and `$EV/interview/leg.stderr` — or, on a night with no child run, the same two files under `<vault>/.brain/interview/leg/` (0700) (INT-01, 2026-09-09) | **yes — allowlisted**, same standing as rows 3 and 4c | 0700 `$EV`, 0600 stderr; the leg's stdout is PIPED into `tools/cos_interview_cli.py` and never written; what it may change is the question TEXT in `<vault>/.brain/interview/state.json` (0600), never an option action |
