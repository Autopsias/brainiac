...
|---|---|---|---|---|---|
| `self_eval` (a) | the run reported a self-eval, and **every id 1..N** the frozen bundle defines | `cos_runverify.check_self_eval` | a skipped self-eval reads as a clean night (run 59) | U | `tests/test_cos_runverify_checks.py::test_self_eval_block_deleted_fires_a` · `tests/test_cos_runverify_checks.py::test_a_partial_self_eval_fires_a` · `tests/test_cos_runverify_checks.py::test_the_right_NUMBER_of_checks_with_the_wrong_IDS_still_fires_a` · `tests/test_cos_runverify_checks.py::test_a_conversation_id_is_not_a_self_eval_result` |
| `self_eval` expected-count derivation (MAN-01) | the count is **frozen into the run manifest at launch** from the bytes that ran; a pre-MAN-01 manifest falls back to the digest-verified re-read, undecidable ⇒ DEGRADED | `cos.write_run_manifest`, `cos_deploy.read_skill`, `cos_runverify.expected_check_count` | scoring a run against a bundle it never executed — and, before the freeze, never scoring it at all: the deployed file has ALWAYS changed by validation time, so every run 101-106 read `degraded` and a run reporting ZERO of its 30 checks scored the same as one reporting all 30 | N/A-cls | `tests/test_cos_runverify_chip_sheet.py::test_the_check_count_is_frozen_at_launch_and_survives_a_new_bundle` · `tests/test_cos_runverify_chip_sheet.py::test_a_pre_MAN01_manifest_still_degrades_rather_than_guessing` (fallback stays honest) |
| instruction sheet (MAN-01) | the host projects `run_id` + `expected_artifacts` + `skill_path`/`skill_sha256` + `lane` into the VM-readable `shared/current-run.json`, so the run **reads** its obligations instead of deriving them; the two producer versions stay host-side | `cos.write_run_manifest`, `cos.current_run_path`, SKILL.md Phase 0 MAN-01 | run 106 named itself off a superseded manifest, composed `_cos_brief_…md` where the manifest declared `_cos_nightly_…md`, and elected `iab` against a `chrome-plugin` pin — one night, three derivations, zero host checks executed | U | `tests/test_cos_runverify_chip_sheet.py::test_the_instruction_sheet_carries_what_the_run_must_obey` · `tests/skills/test_chief_of_staff_navigation.py::test_v558_the_run_id_is_taken_from_the_sheet_never_chosen` · `::test_v558_artifact_names_are_copied_never_composed` · `::test_v558_a_stale_manifest_stops_the_run` |
| stalled-run alert | a run that **wrote artifacts and never completed** is named; an abandoned stamp (no artifacts) and an in-flight run stay silent; scanned by DATE, not by a 5-run window | `cos_runverify.stalled_runs`, `alert` | the loudest failure the validator has is the one it could not see: `alert` reads recorded verdicts and a run that never completes never gets one — runs 74, 75 and 100 are unscored to this day and nothing said so | U, N/A-cls | `tests/test_cos_runverify_chip_sheet.py::test_a_run_that_worked_all_night_and_never_completed_is_named` · `tests/test_cos_runverify_chip_sheet.py::test_the_stalled_scan_does_not_cry_wolf` (three non-findings) · `tests/test_cos_runverify_chip_sheet.py::test_the_stalled_scan_is_dated_not_counted` |
| ~~action-ledger corroboration~~ **RETIRED (s08, 2026-08-16)** | was: an EMPTY action ledger reads as "nothing acted" only when the run's own mutation counters agree. **`_cos_action_ledger_*` has no v7 producer** — the model legs run `--tools "Read,Glob"` with editing denied — so `unledgered_mutations` and the two controls it fed (`unread_touch`, `target_identity`) could only ever pass on zero rows, and all three are OFF the 17-control bar (`cos_runverify.RETIRED_CONTROLS`). **Successor: `cos_runverify.check_mutation_counters`**, which recounts the metrics row against `_cos_undo_ledger_<run>.jsonl` — the artifact the v7 apply DOES write — and returns FAIL on a contradiction, INCONCLUSIVE on all-zero counters beside a non-empty ledger. | `cos_runverify.check_mutation_counters` (retired: `unledgered_mutations`) | run 106 wrote no action ledger while its metrics row recorded 2 verified archives and its report named 5 unrecovered identity mismatches — both controls returned PASS on 0 rows; then run 145 applied 16 mutations against an all-zero row and the successor was built | M | `tests/test_cos_runverify_refusals_undo.py::test_a_mutating_run_whose_counters_were_never_written_is_inconclusive` · `tests/test_cos_runverify_refusals_undo.py::test_a_metrics_row_that_contradicts_the_undo_ledger_is_a_fail` · `tests/test_cos_runverify_refusals_undo.py::test_a_deleted_undo_ledger_can_no_longer_buy_plan_binding_a_pass` · (retired, unscored, still tested: `tests/test_cos_runverify_chip_sheet.py::test_a_mutating_run_with_no_action_ledger_is_inconclusive_not_a_pass`) |
| `metrics_row` (b) | the row exists, carries required fields + host stamps, and its ingestion counters **survive a recount from the run's own ledger** | `cos_runverify.check_metrics_row` | a counter quietly disagreeing with the ledger, or going away | U | `tests/test_cos_runverify_checks.py::test_a_missing_metrics_row_fires_b` · `tests/test_cos_runverify_checks.py::test_a_dropped_metrics_field_fires_b` · `tests/test_cos_runverify_checks.py::test_a_counter_that_disagrees_with_the_ledger_fires_b` · `tests/test_cos_runverify_checks.py::test_a_metrics_row_contradicting_the_manifest_fires_b` · `tests/test_cos_runverify_checks.py::test_an_unstamped_metrics_row_is_degraded_not_valid` |
| `ingestion_ledger` (c) | on a mail-live night the ledger exists and is not vacuous (applicability delegated to the observation guard, §3) | `cos_runverify.check_ingestion_ledger` | a silent Phase 1.6 — the original E16 hole | M, N/A-cls | `tests/test_cos_runverify_checks.py::test_a_mail_live_night_with_no_ledger_fires_c` |
| `body_pass` | a `no-substance` verdict — the one hold reason reachable only by reading the body — is backed by a `body_opened: true` stamp | `cos_runverify.check_body_pass` | run 64's fabricated Phase 1.6: 58 substance verdicts, zero reads. Nothing else could see it — the row count was right and `candidate_stamps` passed vacuously | C | `tests/test_cos_runverify_body_corpus.py::test_a_substance_verdict_without_the_body_read_is_invalid` · `tests/test_cos_runverify_body_corpus.py::test_the_other_hold_reasons_do_not_require_a_body_read` (non-wolf-cry) · `tests/test_cos_runverify_body_corpus.py::test_substance_verdicts_with_no_body_stamp_at_all_are_degraded` · `tests/test_cos_runverify_body_corpus.py::test_run_64_body_pass_against_the_real_ledgers` (real corpus) |
| `body_open_count` (S19) | `body_open_actual` survives a recount of the run's OWN ingestion ledger — the one Phase-1.6 counter nothing joined | `cos_runverify.check_body_open_count` | run 64's row claims 0 opens against 4 stamped rows; a counter for the phase that costs real work, free to be any number | M, N/A-cls | `tests/test_cos_runverify_body_corpus.py::test_a_claimed_open_count_the_ledger_denies_is_invalid` · `tests/test_cos_runverify_body_corpus.py::test_the_disagreement_is_caught_in_the_run_64_direction_too` · `tests/test_cos_runverify_body_corpus.py::test_a_ledger_with_no_body_stamp_is_degraded_not_a_pass` · `tests/test_cos_runverify_body_corpus.py::test_a_run_that_predates_the_counter_is_degraded_not_failed` · `tests/test_cos_runverify_body_corpus.py::test_the_open_count_recount_against_the_real_metrics_rows` (real corpus: 61 and 63 pass, 64 fails) |
| `corpus_join` (WIR-03) | every in-scope ledger row resolves to a thread the run's OWN capture corpus recorded, and every `body_opened: true` row's thread carries corpus TEXT — joined on `conversation_id` against an artifact stronger than the ledger, which run 64 proved can be fabricated | `cos_runverify.check_corpus_join` | a ledger claiming a verdict, or a body read, that the independent corpus never captured | M, N/A-cls | `tests/test_cos_runverify_body_corpus.py::test_corpus_join_fires_on_a_ledger_row_the_corpus_never_captured` · `tests/test_cos_runverify_body_corpus.py::test_corpus_join_fires_on_a_claimed_open_the_corpus_holds_no_text_for` · `tests/test_cos_runverify_body_corpus.py::test_corpus_join_is_not_applicable_when_this_run_wrote_no_corpus` (non-wolf-cry) · `tests/test_cos_runverify_body_corpus.py::test_corpus_join_passes_on_a_healthy_run_with_a_matching_corpus` · `tests/test_cos_runverify_body_corpus.py::test_corpus_join_zero_false_positives_across_every_real_historical_run` (real corpus: every run with no corpus on disk must be "not applicable"; run 68 is this host's first TRUE positive — a premature `rows: 0` close, since recoverable via `cos-corpus-reopen`) |
| `artifact_naming` | every EVIDENCE artifact naming this run carries the date the HOST assigned; the morning-dated brief and decision card are exempt | `cos_runverify.check_artifact_naming` | run 64 crossed midnight, named nine artifacts from the local clock and repaired with `cp` — `cos_reconcile_metrics` then counted the duplicates as extra work | U, N/A-cls | `tests/test_cos_runverify_body_corpus.py::test_a_second_date_prefix_on_this_run_is_invalid` · `tests/test_cos_runverify_body_corpus.py::test_the_morning_brief_may_carry_the_morning_after_date` (non-wolf-cry) |
| `candidate_stamps` | E16's stamp clause where v5.39 put it: proposal id + full content digest + a real overlay category | `cos_runverify.check_candidate_stamps` | run 59's 8 unstamped candidates claimed anyway | C | `tests/test_cos_runverify_checks.py::test_an_invented_category_fires_the_stamp_check` · `tests/test_cos_runverify_checks.py::test_a_duplicate_proposal_id_fires_the_stamp_check` · `tests/test_cos_runverify_checks.py::test_a_candidate_without_a_digest_is_degraded_not_valid` |
| `contract` (d) | the outcome-contract verdict is present, checker-produced and **re-derivable** — the one control that fully re-executes | `cos_runverify.check_contract` | a hand-written verdict standing in for a computed one | U | `tests/test_cos_runverify_checks.py::test_a_missing_contract_block_fires_d` · `tests/test_cos_runverify_checks.py::test_a_hand_composed_verdict_fires_d` · `tests/test_cos_runverify_checks.py::test_an_edited_verdict_does_not_survive_re_execution` · `tests/test_cos_runverify_checks.py::test_contract_inputs_the_checker_refuses_are_not_a_verdict` |
| degrade consistency | a degrade claim corroborated across artifacts the run does **not** control (host-received candidate sidecars) | `cos_runverify.degrade_evidence`, `check_degrade_consistency` | a skip wearing a degrade marker | M | `tests/test_cos_runverify_checks.py::test_the_adversarial_fixture_a_skip_wearing_a_degrade_marker` · `tests/test_cos_runverify_checks.py::test_a_degrade_marker_beside_candidates_the_host_received_fails` · `tests/test_cos_runverify_checks.py::test_signed_out_degrade_is_reported_degradation_not_a_validator_failure` |
| completion + quiescence | an in-flight run is PENDING, never a verdict; artifacts are name-scoped to the run | `cos_runverify.completion`, `run_artifacts` | crying wolf on every early run — the way a guard gets muted | N/A-cls | `tests/test_cos_runverify_checks.py::test_an_incomplete_artifact_set_is_pending_never_a_verdict` · `tests/test_cos_runverify_checks.py::test_a_run_still_being_written_is_pending` · `tests/test_cos_runverify_checks.py::test_run_5_does_not_swallow_run_59s_artifacts` |
| input digest / re-validation | a verdict is bound to the inputs it was computed from; a late or swapped artifact re-scores | `cos_runverify.inputs_digest`, `verify_pending_runs` | a cached PASS over a partial artifact set | U | `tests/test_cos_runverify_checks.py::test_scoring_is_idempotent_but_re_fires_when_an_artifact_changes` · `tests/test_cos_runverify_checks.py::test_a_late_metrics_row_re_validates_rather_than_standing_invalid` |
| fail-closed INCONCLUSIVE | no manifest / no ops dir / **no checkers on disk** ⇒ INCONCLUSIVE, never a pass | `cos_runverify.verify_run`, `checkers` | "nothing to validate" reported as everything passing (shape B) | U | `tests/test_cos_runverify_checks.py::test_a_run_with_no_manifest_is_inconclusive` · `tests/test_cos_runverify_checks.py::test_a_missing_ops_dir_is_inconclusive_not_a_pass` · `tests/test_cos_runverify_checks.py::test_no_checkers_on_disk_is_inconclusive_not_a_pass` |
| consequence + loudness | an INVALID run's candidates are **quarantined, not claimed**, and the failure surfaces on every surface | `cos_runverify.alert`, `hot_entry` | a verdict that only lands in a log — the silent-instrument shape again | U | `tests/test_cos_runverify_checks.py::test_an_invalid_run_quarantines_its_candidates_instead_of_claiming` · `tests/test_cos_runverify_checks.py::test_the_failure_is_loud_on_every_surface` |
...
| real-corpus probe | the same validator over the real vault's artifacts, skipped when absent | `tests/test_cos_runverify.py` | fixtures diverging from field shapes | N/A-cls | `tests/test_cos_runverify_checks.py::test_known_positive_against_the_real_vault` |
...
|---|---|---|---|---|
| `OC-a-unaccounted` | every enumerated conversation lands in an accounted bucket for its profile | a thread silently vanishing from the accounting | U | `tests/test_cos_contract_accounting.py::test_one_unaccounted_convid_is_failed_even_with_everything_else_clean` · `tests/test_cos_contract_accounting.py::test_a_bare_chip_is_not_accounted_under_the_full_profile` |
| `OC-scope-violation-archived-under-label-only` | a label-only run archived nothing | scope creep past the declared profile | C | `tests/test_cos_contract_accounting.py::test_label_only_treats_an_archive_as_a_scope_violation` |
| `OC-degenerate` | an all-held run is not passed off as work | 27/27 E-checks over seven days of nothing | U | `tests/test_cos_contract_accounting.py::test_degenerate_all_held_run_trips_the_anti_degenerate_guard` (+ non-wolf-cry: `tests/test_cos_contract_accounting.py::test_safety_frozen_archive_candidates_are_not_degenerate`) |
| `OC-liveness:<cap>` | each in-scope capability actually produced verified ledger output **this run** | a dead capability inside a green run | U | `tests/test_cos_contract_accounting.py::test_dead_drafting_capability_fails_while_every_accounting_clause_passes` · `tests/test_cos_contract_accounting.py::test_an_omitted_capability_is_failed_not_ignored` · `tests/test_cos_contract_accounting.py::test_an_in_scope_claim_inconsistent_with_the_profile_is_failed` · `tests/test_cos_contract_accounting.py::test_a_forged_eligible_inputs_zero_does_not_buy_a_pass` · `tests/test_cos_contract_accounting.py::test_yesterdays_ledger_never_satisfies_todays_liveness_guard` · `tests/test_cos_contract_accounting.py::test_eligible_deadline_rows_alone_still_fail_a_zero_draft_run` |
| `OC-candidate-no-exclusion-reason` | an ineligible candidate states why | silent exclusion | C | `tests/test_cos_contract_accounting.py::test_an_ineligible_record_without_a_reason_is_failed` |
| `OC-provenance-bucket-sum` | buckets sum to the enumerated set | arithmetic that hides a thread | U | `tests/test_cos_contract_accounting.py::test_buckets_that_do_not_sum_to_the_enumerated_set_are_failed` |
| `OC-provenance-residency` | residency matches the post-run inbox count | a POST snapshot that does not describe the mailbox | U | `tests/test_cos_contract_accounting.py::test_residency_must_match_the_post_run_inbox_count` (+ `tests/test_cos_contract_accounting.py::test_archived_rows_left_the_inbox_so_residency_excludes_them`, `tests/test_cos_contract_accounting.py::test_owa_message_items_are_independent_of_conversation_residency`) |
| `OC-provenance-folder-count` | legacy folder count agrees | same, legacy schema | U | `tests/test_cos_contract_accounting.py::test_a_transcribed_folder_count_that_disagrees_is_failed` |
| `OC-provenance-unknown-convid` | no post-run row for a conversation never enumerated and never arrived | invented rows | U | `tests/test_cos_contract_accounting.py::test_a_convid_that_was_never_enumerated_and_never_arrived_is_failed` (+ `tests/test_cos_contract_accounting.py::test_mail_arriving_after_enumeration_is_a_delta_never_a_contract_miss`) |
| `OC-provenance-incomplete-enumeration` | enumeration was complete and says so explicitly | a partial scan scored as a full inbox | U | `tests/test_cos_contract_enumeration.py::test_incomplete_new_schema_enumeration_can_never_pass` · `tests/test_cos_contract_enumeration.py::test_new_schema_requires_explicit_complete_enumeration_proof` · `tests/test_cos_contract_enumeration.py::test_enumeration_proof_must_reconcile_to_the_list_declared_size` |
| `OC-provenance-pre-enumeration-count` | the PRE count equals the serialized enumeration | a count asserted beside a shorter list | U | `tests/test_cos_contract_enumeration.py::test_pre_conversation_count_must_match_the_serialized_enumeration` |
| `ZS-new-sent-item` | no new Sent item id between PRE and POST | a send inside a "zero-send" run | U | `tests/test_cos_contract_enumeration.py::test_new_sent_item_id_fails_even_when_sent_count_is_unchanged` |
| `ZS-incomplete` | the Sent prefix is complete enough to prove zero-send | an incomplete list proving nothing | U | `tests/test_cos_contract_enumeration.py::test_incomplete_sent_prefix_can_never_prove_zero_send` (+ bound: `tests/test_cos_contract_enumeration.py::test_bounded_recent_sent_prefix_does_not_need_full_folder_set_size`) |
| `ZS-window-mismatch` | PRE and POST measured the **same** Sent window | a send in the gap sits in neither list and the id diff comes back empty — zero-send by not looking | U | `tests/test_cos_contract_enumeration.py::test_a_post_sent_window_that_moved_can_never_prove_zero_send` **(added by this sweep)** |
| `validate()` malformed inputs | the checker refuses inputs it cannot score, rather than rendering a verdict | a verdict computed over garbage | U | `tests/test_cos_contract_enumeration.py::test_malformed_inputs_raise_rather_than_render_a_verdict` · `tests/test_cos_contract_enumeration.py::test_new_count_schema_is_all_or_nothing` · `tests/test_cos_contract_enumeration.py::test_candidate_records_require_an_enumerated_convid` · `tests/test_cos_contract_enumeration.py::test_duplicate_candidate_for_same_conversation_and_capability_is_malformed` |
| scan/browser provenance | IAB-first election, fresh same-run scans | a verdict over yesterday's or another lane's scan | U | `tests/test_cos_contract_enumeration.py::test_new_schema_requires_iab_first_fresh_same_run_scan_provenance` |
| `preflight()` | the serialized PRE snapshot is valid **before any mutation** | mutating on an unverifiable baseline | U | `tests/test_cos_contract_enumeration.py::test_preflight_rejects_truncated_serialization_before_mutation` |
| CLI exit contract | 0 pass / 1 failed / 2 malformed; a run id is required | a wrapper reading "no output" as success | U | `tests/test_cos_contract_reporting.py::test_cli_exit_codes_are_zero_pass_one_failed_two_malformed` · `tests/test_cos_contract_reporting.py::test_cli_requires_a_run_id` |
| healthy-run direction | a real full run passes and reports its split (non-wolf-cry) | a gate so loud it gets muted | U | `tests/test_cos_contract_accounting.py::test_a_healthy_full_run_passes_and_reports_the_split` · `tests/test_cos_contract_accounting.py::test_a_label_only_run_whose_holds_rose_with_zero_archives_passes` · `tests/test_cos_contract_accounting.py::test_an_ineligible_only_run_passes_with_zero_output` |
...
|---|---|---|---|---|
| `reconcile` / `shortfalls` | every counter in the metrics row is **recounted from the ledgers**; under-report ⇒ non-zero exit. **(s10, 2026-08-16) `_cos_undo_ledger_*` joined.** The three globs this join was born for are pre-v7 MODEL-written and have no v7 producer, so on a v7 day it compared a positive reported total against 0 ledgered and `shortfall = max(0, ledgered − reported)` was 0 for every counter — measured 2026-08-16, `archived` reported 14 / ledgered **0**, and 0 shortfalls on any v7 date. With the undo ledger joined the same scan reports 14 real shortfalls across six v7 dates. The pre-v7 globs are KEPT (their files are on disk and still report real historical shortfalls); one mutation is an idempotency KEY, not a row. | 181 verified archives reported as 0 (2026-07-21); 11 archives + 3 marks + 2 drafts reported as 0 (run 145, 2026-08-16) | C | `tests/test_cos_metrics_reconcile.py::test_ledgered_draft_and_zero_counter_can_never_coexist` · `tests/test_cos_metrics_reconcile.py::test_marked_and_archived_share_the_defect_and_the_gate` · `tests/test_cos_metrics_reconcile.py::test_a_mutating_run_with_no_metrics_row_at_all_is_caught` · `tests/test_cos_metrics_reconcile.py::test_reconciler_cli_exits_nonzero_on_a_shortfall` · `tests/test_cos_metrics_reconcile.py::test_a_v7_apply_the_metrics_row_never_reported_is_a_shortfall` · `tests/test_cos_metrics_reconcile.py::test_the_same_undo_ledger_reconciles_once_the_apply_records_it` (known positive) · `tests/test_cos_metrics_reconcile.py::test_an_aborted_mutation_is_not_ledgered_work` · `tests/test_cos_metrics_reconcile.py::test_one_mutation_is_a_key_not_a_row` · `tests/test_cos_metrics_reconcile.py::test_a_read_only_night_writes_no_undo_ledger_and_reports_nothing` (non-wolf-cry) |
| verified-only counting | only `verified-*` rows count as executed; a re-verification is not a creation | credit for unverified mutations | C | `tests/test_cos_metrics_reconcile.py::test_unverified_rows_never_count_as_executed` · `tests/test_cos_metrics_reconcile.py::test_reverifying_an_earlier_runs_draft_is_not_a_creation` · `tests/test_cos_metrics_reconcile.py::test_a_run_that_reports_its_draft_reconciles_clean` |
| `_require_ingestion_fields` (refusal) | an append **without** the four Phase-1.6 fields is refused; `null` counts as absent; unknown `attachment_lane` refused | a counter quietly going away (it did, at run 41) | U | `tests/test_cos_metrics_reconcile.py::test_append_refuses_a_row_that_drops_an_ingestion_field` · `tests/test_cos_metrics_reconcile.py::test_append_refuses_an_unknown_attachment_lane` |
| `append_metric` (refusal) | append is idempotent by `(date, run)` and refuses a **conflicting** row for the same key; date+run required | two contradictory records for one run | U | `tests/test_cos_metrics_reconcile.py::test_metrics_append_is_idempotent_and_refuses_conflicting_run_rows` · `tests/test_cos_metrics_reconcile.py::test_metrics_append_requires_a_date_and_run_key` |
| `counts_ingestion` join | `candidate` rows only; held/no-substance/`zero-eligible` count zero honestly | a held ledger read as production, or as absence | C | `tests/test_cos_metrics_reconcile.py::test_ingestion_ledger_candidates_are_joined_like_every_other_counter` · `tests/test_cos_metrics_reconcile.py::test_an_all_held_ingestion_ledger_reconciles_clean_at_zero` · `tests/test_cos_metrics_reconcile.py::test_a_zero_eligible_marker_row_counts_zero` |
...
| `host_stamps` | the metrics row's version stamps come from the **host** run manifest, not the producer | a run reporting a bundle it did not execute | U | `tests/test_cos_runverify_checks.py::test_a_metrics_row_contradicting_the_manifest_fires_b` |
...
| `cos_deployed_version` lane resolution | resolves which surface **actually executes** the nightly; an inactive automation, an automation naming a missing file, and a backup automation file all fail to resolve a lane | the original shape-B defect: reading a non-executing surface, 2 false freeze alarms | U | `tests/test_cos_deployed_version.py::test_an_inactive_automation_does_not_resolve_the_lane` · `tests/test_cos_deployed_version.py::test_an_automation_naming_a_missing_file_does_not_resolve_the_lane` · `tests/test_cos_deployed_version.py::test_backup_automation_files_are_not_read_as_config` · `tests/test_cos_deployed_version.py::test_the_stale_desktop_store_no_longer_manufactures_a_mismatch` |
| `cos_deployed_version` refusal | refuses to answer when no lane resolves; an unknown lane and a missing ops dir are usage errors, not answers | "no lane" reported as agreement | U | `tests/test_cos_deployed_version.py::test_refuses_to_answer_when_no_lane_can_be_resolved` · `tests/test_cos_deployed_version.py::test_an_unknown_lane_is_a_usage_error` · `tests/test_cos_deployed_version.py::test_missing_cos_ops_dir_is_a_usage_error` |
| `cos_deployed_version --expect` | a version found on a non-executing surface never satisfies `--expect` | a pin move gated on a claim instead of a readback | U | `tests/test_cos_deployed_version.py::test_a_version_on_a_non_executing_surface_never_satisfies_expect` · `tests/test_cos_deployed_version.py::test_explicit_lane_lets_the_operator_assert_the_desktop_store` |
| run-report readback | both report wordings parse; "no version stated" is not a version | a phrase read as a version number | U | `tests/test_cos_deployed_version.py::test_both_run_report_wordings_are_recognised` · `tests/test_cos_deployed_version.py::test_a_run_report_that_states_no_version_is_not_a_version` |
...
| retirement is reversible by the owner, not by a flag | a store that matches the executing lane answers normally again | a retirement nobody can undo, so it gets worked around | U | `tests/test_cos_deployed_version.py::test_re_uploading_the_current_bundle_makes_the_surface_answerable_again` |
| run manifest never stamps from the retired surface | `cos_deploy.deployed_skill(lane="cowork-desktop")` raises `SurfaceUnsupported` | a run stamped with a version that never executed | U | `tests/test_cos_deployed_version.py::test_the_run_manifest_never_stamps_from_the_retired_surface` |
| `brain doctor` deployed-skill row (DEP-02) | reports the executing lane, version, extraction-rules version, digest and path — and names the retired store when it disagrees; **never** gates the exit code | "which version runs tonight" answerable only by knowing which surface to read | N/A-info | `tests/test_doctor_cos.py::test_cos_deployed_skill_row_names_the_executing_lane` · `tests/test_doctor_cos.py::test_cos_deployed_skill_row_is_not_detectable_without_a_deployment` (known-negative: no COS deployment is HEALTHY) · `tests/test_doctor_cos.py::test_cos_deployed_skill_row_never_gates_the_exit_code` |
...
|---|---|---|---|---|
| every negative probe reaches the gate | each `must_refuse` invocation is well-formed enough to reach the role check — a usage error or crash is an **INVALID PROBE**, never a pass | the 2-of-14 vacuous probes | U | `tests/test_vm_boundary_probe.py::test_every_negative_probe_reaches_the_role_gate` · `tests/test_vm_boundary_probe.py::test_a_non_zero_exit_alone_is_not_scored_as_a_refusal` |
| refusal recognition | the probe's regex matches the engine's **real** refusal message | a changed message turning every probe vacuous | U | `tests/test_vm_boundary_probe.py::test_the_refusal_regex_matches_the_engines_real_message` |
| coverage of the host-only surface | every named host-only verb, and every host-only `cos-*` verb, is probed | a new privileged verb shipping unprobed | U | `tests/test_vm_boundary_probe.py::test_the_named_host_only_verbs_are_all_probed` · `tests/test_vm_boundary_probe.py::test_every_host_only_cos_verb_is_probed` |
| no false negatives | no probe asserts a refusal for a VM_ALLOWED verb; the one VM-allowed `cos-*` verb is exercised as a **positive** | a probe suite that would pass on a broken engine | U | `tests/test_vm_boundary_probe.py::test_no_probe_asserts_a_refusal_for_a_vm_allowed_verb` · `tests/test_vm_boundary_probe.py::test_the_one_vm_allowed_cos_verb_is_exercised_as_a_positive` |
| observation vs breach | host-private readability is reported as an **observation**, not scored as a breach | crying wolf on the mount's own geometry | N/A-cls | `tests/test_vm_boundary_probe.py::test_host_private_readability_is_an_observation_not_a_breach` |
| unimportable engine | an engine that will not import is INCONCLUSIVE, never a pass (the `ModuleNotFoundError` probe) | shape B | U | `tests/test_vm_boundary_probe.py::test_an_unimportable_engine_is_inconclusive_not_a_pass` |
| script integrity | the script is syntactically valid bash | a probe file that never ran at all | U | `tests/test_vm_boundary_probe.py::test_the_script_is_syntactically_valid_bash` |
| **durable staging** (DEP-02, 0.19.22) | the probe rides the wheel (`ENGINE_ASSET_FILES`) and is staged `0755` into `<workspace>/vault/.brain/` by BOTH staging paths — `brain update` and `tools/cowork_workspace_install.sh` | the caveat below, now closed: a hand copy the next re-stage deletes, leaving a boundary claim nobody can re-measure | U | `tests/test_update_venv_stage.py::test_stage_stages_vm_boundary_probe_executable` · `tests/test_update_venv_stage.py::test_cowork_installer_stages_the_boundary_probe_too` · lockstep: `tools/package_clients.py --validate-only` (proven able to fail — tampering with the `_assets` mirror copy exits 1 with `engine asset mirror stale`) |
...
|---|---|---|---|---|
| bitemporal per-note types | ISO dates, real booleans, resolvable ids | `tools/validate.py::check_bitemporal_note` | C | `tests/test_bitemporal.py::test_malformed_date_is_error` · `tests/test_bitemporal.py::test_legacy_note_with_no_bitemporal_keys_still_validates_clean` |
...
| supersession: retirement completeness | `is_latest_version: false` requires `superseded_by` | same | C | `tests/test_bitemporal.py::test_is_latest_version_false_requires_superseded_by` |
| supersession: no self-supersession | a note cannot supersede itself | same | C | `tests/test_bitemporal.py::test_self_supersession_is_error` |
| supersession: no cycles | the chain is acyclic | same | C | `tests/test_bitemporal.py::test_cycle_is_error` |
| supersession: no forks | two successors cannot claim one predecessor | same | C | `tests/test_bitemporal.py::test_fork_two_successors_same_predecessor_is_error` |
| supersession: one latest per chain | at most one `is_latest_version: true` | same | C | `tests/test_bitemporal.py::test_more_than_one_latest_in_chain_is_error` |
| supersession: classification on both sides | both ends of every link carry an explicit `classification` | same | C | `tests/test_bitemporal.py::test_successor_missing_classification_in_chain_is_error` · `tests/test_bitemporal.py::test_predecessor_missing_classification_in_chain_is_error` |
| supersession: warn-only reciprocity | a missing reciprocal link WARNS, never errors | same | C | `tests/test_bitemporal.py::test_missing_reciprocal_link_is_warn_only` |
| valid-chain direction | a correct chain validates clean (non-wolf-cry) | same | C | `tests/test_bitemporal.py::test_valid_supersession_chain_validates_clean` |
...
| type lint (warn-only) | unrecognised `type:`, `concept` without counter-arguments, `decision` without a source anchor | `validate.py::check_type_lint` | C | `tests/test_typed_entities.py::test_unrecognized_type_warns_not_errors` · `tests/test_typed_entities.py::test_concept_without_counter_arguments_warns` · `tests/test_typed_entities.py::test_decision_without_source_anchor_warns` |
...
| state-MOC section staleness | a stamped section past the threshold warns; warn-only never blocks | `validate.py::check_section_staleness` | C | `tests/test_zone_catalogs.py::test_section_one_day_past_threshold_is_flagged` · `tests/test_zone_catalogs.py::test_section_within_threshold_is_not_flagged` · `tests/test_zone_catalogs.py::test_staleness_lint_is_warn_only_never_blocks_the_gate` |
| **release contamination scan** | denylist terms absent from the export tree; **comments/blanks stripped first** (the 0.16.0 defect), missing denylist refused, `--denylist` required | `tools/publish_release.py::step_contamination_scan` | U | `tests/test_publish_release.py::test_step_contamination_scan_detects_a_real_hit` (the known positive) · `tests/test_publish_release.py::test_step_contamination_scan_reports_zero_hits_for_a_denylist_term_absent_from_export` · `tests/test_publish_release.py::test_step_contamination_scan_refuses_missing_denylist` · `tests/test_publish_release.py::test_contamination_scan_refuses_without_denylist_flag` |
| release evidence pass | evidence hits are **informational**, labelled as such, and never fail the run | `publish_release.py` | N/A-cls | `tests/test_publish_release.py::test_evidence_hits_are_informational_only_and_never_fail_main` · `tests/test_publish_release.py::test_evidence_hit_print_is_labeled_informational_not_expected_zero` |
