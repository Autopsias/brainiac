...
| `self_eval` expected-count derivation | the count comes from the **manifest's frozen skill digest**, not today's file; undecidable ⇒ DEGRADED | `cos_runverify.expected_check_count` | scoring a run against a bundle it never executed | N/A-cls | `tests/test_cos_runverify.py::test_the_expected_count_comes_from_the_manifest_not_todays_skill` |
