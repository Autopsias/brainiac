...
`graph-expand`, `get`/`read`, `recent`, `curate`/`integrity`/`promote-scan`,
`brief`/`digest` — routes through ONE helper (`brain.egress.apply_gate`),
shared by the CLI and the optional MCP adapter. A new content path cannot
silently bypass a gate a sibling path enforces. Proven per-subcommand in
`_evidence/s08/egress-per-subcommand.txt` + `tests/test_egress_per_subcommand.py`
(the canonical list is asserted exhaustive).

### brief/digest gate + VM-membership decision (H-1, s02)

`brain brief`/`brain digest` (UX-02) built their note list from the SAME
`recent()` feed as `recent`, but returned it straight to `_emit` with no
`apply_gate` call and were absent from `CONTENT_RETURNING_SUBCOMMANDS` — a
posture gap: `brain brief --json` / `brain digest --json` surfaced
Restricted/MNPI titles+paths+classification with no `--max-tier` elevation,
from both host and VM. Fixed by routing the `recent()` list through
`egress.apply_gate` inside `BrainCore.brief`/`BrainCore.digest` (before it is
assembled into the brief/digest structure — same chokepoint, no second gate),
adding `brief`/`digest` to `CONTENT_RETURNING_SUBCOMMANDS`, and wiring
`--max-tier` on both CLI subcommands. Covered by
`tests/test_egress_per_subcommand.py::test_brief_digest_gate_on_host` and
`::test_brief_digest_gate_on_vm`.

**VM-membership decision (explicit, not left implicit):** brief/digest STAY in
`brain.cli.VM_ALLOWED`. Gating output and VM membership are separate
questions — now that the note list is gated identically to `recent`/`search`
(read-only, no signing key, deny-by-default filter applied before assembly),
brief/digest are exactly as safe on the VM leg as those verbs. Revisit only if
a future brief/digest field starts drawing from an ungated source (e.g. raw
audit/WAL internals) — that needs its own gate or a host-only demotion, not a
blanket `VM_ALLOWED` removal for the whole command.
...
egress of vault content and MNPI specifically**.
...
- Openness ≠ "any app." A harness is **ALLOWED** to run against the **full vault**
  ...
  tool-call/API egress of vault content **and** MNPI (the bar is the register's
  `bar` array).
...
|---|---|---|---|---|--:|
| claude-desktop | Anthropic | **PENDING** | the organization's Legal confirm the signed Anthropic enterprise agreement's no-train + ZDR clauses cover tool-call egress of vault content + MNPI | maintainer / the organization's Legal | ~3 wks |
| codex-cli | OpenAI | **PENDING** | Confirm OpenAI tenant no-train default applies to Codex CLI tool-call egress + ZDR covers MNPI | maintainer / the organization's Cyber | ~3 wks |
| gemini-cli | Google | **PENDING** | Confirm the Workspace/Vertex tier is a no-train **paid** tier (NOT consumer Gemini) + DPA covers tool-call egress + MNPI | maintainer / the organization's Cyber | ~3 wks |
