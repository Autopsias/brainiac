...
|---|---|---|---|---|--:|
| claude-desktop | Anthropic | **PENDING** | Acme Legal confirm the signed Anthropic enterprise agreement's no-train + ZDR clauses cover tool-call egress of vault content + MNPI | maintainer / Acme Legal | ~3 wks |
| codex-cli | OpenAI | **PENDING** | Confirm OpenAI tenant no-train default applies to Codex CLI tool-call egress + ZDR covers MNPI | maintainer / Acme Cyber | ~3 wks |
| gemini-cli | Google | **PENDING** | Confirm the Workspace/Vertex tier is a no-train **paid** tier (NOT consumer Gemini) + DPA covers tool-call egress + MNPI | maintainer / Acme Cyber | ~3 wks |
