...
execute**. They require a named human owner inside Example Corp, have real lead times,
...
> individual Example Corp staff are intentionally left as role labels here per the vault
...
|---|---|---|---|--:|---|---|
| PW-1 | **Corporate cyber-review of the NIST CSF 2.0 profile** (sign-off) | the maintainer / Example Corp Cyber (CISO team) | ~3–4 wks (booking + review cycle) | **PENDING** | pilot expansion; s07 checkpoint | The CSF profile (`nist-csf-2.0-profile.md`) is **cyber-REVIEWED, not self-attested**. Reviewer identity, dated outcome, open exceptions + risk owner, and pilot/fleet conditions are filled in by the reviewer — see the profile's "Sign-off" block. **Abort branch:** if the review REJECTS the egress posture → halt + stay on Obsidian+SC (first-class outcome, below). |
| PW-2 | **Azure Trusted Signing onboarding** (code-signing identity for the Windows/Mac `brain` binaries) | the maintainer / Example Corp EUC + Cyber | ~2–3 wks (tenant onboarding + identity validation) | **PENDING** | pkg-03 signed binaries; WDAC Managed-Installer trust | Without a signed binary the WDAC Managed-Installer policy (PW-3) has nothing to trust and the Keychain ACL "signed designated requirement" leg of SEC-02 is weaker. |
| PW-3 | **WDAC Managed-Installer policy** (allow the signed `brain` installer on the Example Corp Windows image) | the maintainer / Example Corp EUC (Intune/endpoint) | ~2–3 wks (policy authoring + ring test) | **PENDING** | application-control control (PR.PS); Cowork-Windows surface | Depends on PW-2 (a signing identity to anchor the rule). |
| PW-4 | **Intune test device** (a managed Windows endpoint to validate packaging + Defender + the Managed-Installer policy + Cowork-Windows) | the maintainer / Example Corp EUC (Intune) | ~1–2 wks (device provisioning) | **PENDING** | pkg-03 install validation; val-02 as-built | The AS-BUILT scheduled-task + install evidence is a later (s09 build → s10/val-02 acceptance) artifact; this device is the surface it runs on. |
...
not knowable here (clearly marked; not invented), with the responsible Example Corp team
...
|---|---|---|---|---|---|--:|---|
| PW-1 | Corporate cyber-review of the NIST CSF 2.0 profile (sign-off) | maintainer / **Example Corp Cyber (CISO) — TBD named** | ~3–4 wks | TBD-kickoff | TBD-kickoff+4w | **PENDING** | pilot expansion; **abort-branch decision** |
| PW-2 | Azure Trusted Signing onboarding (+ **region-gating check**: US/CA/EU/UK only) | maintainer / **Example Corp EUC + Cyber — TBD named** | ~2–3 wks | TBD-kickoff | TBD-kickoff+3w | **PENDING** | signed binaries; WDAC trust anchor; SEC-02 signed-DR leg |
| PW-3 | WDAC Managed-Installer policy (trust IME on the Example Corp image) | maintainer / **Example Corp EUC (Intune/endpoint) — TBD named** | ~2–3 wks | TBD (after PW-2) | TBD+3w | **PENDING** | application-control (PR.PS); one-step IT approval |
| PW-4 | Intune test device (validate packaging + Defender + Managed-Installer + Cowork-Windows) | maintainer / **Example Corp EUC (Intune) — TBD named** | ~1–2 wks | TBD-kickoff | TBD-kickoff+2w | **PENDING** | **val-01** clean-install acceptance; Defender/ASR evidence |
...
Example Corp tenant** (no signing identity / WDAC cannot trust the binary), the defined
outcome is:
