...
1. `Developer ID Application: YourOrg (<TEAMID>)` cert in the login keychain.
2. notarytool creds: `xcrun notarytool store-credentials yourorg-notary --apple-id
   <id> --team-id <TEAMID> --password <app-specific-pw>`.

Run:
```bash
DEV_ID="Developer ID Application: YourOrg (TEAMID)" \
NOTARY_PROFILE=yourorg-notary \
