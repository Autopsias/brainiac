...
1. `Developer ID Application: Acme Energia (<TEAMID>)` cert in the login keychain.
2. notarytool creds: `xcrun notarytool store-credentials Acme-notary --apple-id
   <id> --team-id <TEAMID> --password <app-specific-pw>`.

Run:
```bash
DEV_ID="Developer ID Application: Acme Energia (TEAMID)" \
