...
> the end is **PENDING** an external Acme Cyber review (external pre-work PW-1).
...
| **ID.AM-08** Data classification | 5-tier scheme (`Public<Internal<Confidential<Restricted<MNPI`), required on every note, deny-by-default | same | `docs/classification-scheme.md`; `src/brain/classification.py`; `tools/validate.py` |
...
| **DE.AE (independent attestation)** | **Off-host anchor** of the signed chain head → a fully-re-signed silent rewrite by the key-holder is **detectable** (internal verify misses it; the anchor catches it) | scheduled daily anchor + optional RFC-3161 TSA | `src/brain/anchor.py`; `_evidence/s08/anchor-demo.txt`; `tests/test_anchor.py` |
...
/ WDAC trust (PW-2/PW-3) **cannot be obtained** for the Acme tenant:
...
| Field | Value |
|---|---|
| Reviewer (identity / team) | _PENDING_ — Acme Cyber (CISO team), named individual at booking |
