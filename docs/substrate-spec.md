...
4. **Classification everywhere, default-deny.** Every note carries
   `classification:`; unlabelled ⇒ treated as Secret (most-restrictive) at the
...
tests in `tests/test_s06_integration.py`.
...
in the recognised set is treated as **Secret** and withheld. Full scheme,
...
- **Conditional app-encryption** (age/SOPS/SQLCipher, ACL-bound Keychain key)
  ...
  backup/sync, regulated data (PCI/Secret/PII regime), multi-user machine, or a
