...
## Step 0 — sandbox self-check (route, don't silently degrade)

If this shell does not run directly on the user's own machine (e.g. you are
Cowork, a cloud/remote agent, or any ephemeral VM/sandbox) — do NOT run the
installer here; anything it puts on PATH gets wiped and never reaches the
user's real terminal. Route by what the user wants:

**They want a NEW vault, from this sandbox (PRV-10 — the normal Cowork
path, works whenever the host already runs at least one Brainiac vault):**
...
4. Tell the user: the HOST completes provisioning on its next hourly
   ...
   `vault/.brain/provision-result.json` — read it back to them when it
   lands. Do NOT try to work around the missing host steps yourself (no
   key minting, no launchd, no registry writes from a sandbox).

**They want the FIRST Brainiac install on a machine with none:** the drain
has no nightly to ride there — print exactly this block for the user to
paste into their own terminal, then wait for them to report back:
...
Cowork runtime staging into an EXISTING workspace remains
`/brainiac-cowork-setup` (host-side) — never conflate the three.
...
This places `~/.claude/hooks/brainiac-alerts.sh` and registers it as a
`SessionStart` hook, so every session opens with the `brain alerts` banner
instead of relying on the model to remember the AGENTS.md line. Idempotent —
re-running reports `already-registered`. It only ever ADDS its own entry and
never touches `permissions` or any other settings key.

Report its two lines verbatim. Three outcomes are NOT success and must be
said plainly: `settings: unparseable` means the owner's `settings.json` could
not be parsed and was left untouched (they add the entry by hand, or fix the
JSON and re-run); `script: missing` means the packaged hook could not be
resolved, so nothing was registered; and
`settings: harness-managed-UNREGISTERED` means a harness repo deploys this
`~/.claude`, so Brainiac will not write its `settings.json` — and no
SessionStart entry exists, so the owner must add it in that repo.

**On a harness-managed `~/.claude` this writes NOTHING but the script**
(owner ruling 2026-08-20). A deploy target has exactly one writer: the engine
ships its own artifact, the harness declares the wiring. It reports
`settings: harness-managed` and moves on. A standalone host registers itself
as normal.

`brain update` re-runs this same leg on every update, so a host carrying an
older copy of the hook is repaired there.

**Codex is separate and stays manual.** `~/.codex/hooks.json` takes the same
...
✅/❌ session-start alert hook: registered | already-registered (~/.claude/settings.json)
```
