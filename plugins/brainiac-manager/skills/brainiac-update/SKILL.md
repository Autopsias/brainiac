...
This skill is a thin driver over `brain update` (ADR-0005 Ruling 3 + the
2026-07-25 v2 amendment, UP-01/UP-02 — `src/brain/update.py`, dispatched by
`src/brain/cli.py`). The engine does the actual work; this skill's job is to
re-resolve the checkout, invoke `brain update`, and relay its report. It never
reimplements the version compare, the reinstall decision, or the doctor verify
— those are tested Python (`tests/test_update.py`, `tests/test_doctor.py`), not
skill prose.

**You will usually have nothing to do.** As of the v2 amendment the hourly
`brain-nightly` host task AUTO-APPLIES a newer version unattended (post-update
`doctor`-green + real-embed gated; a failed version is not auto-retried), and a
session-start banner (`~/.brainiac/update-state.json` → `scripts/brainiac-alerts.sh`)
already tells the owner when an update applied, failed, or is waiting. Run this
skill when the owner explicitly asks, when the banner says an auto-update
FAILED, or to force a re-stage. The channel resolution is now installLocation-
first (`known_marketplaces.json`), so the sixth channel `venv-wheel` (a plain
wheel in `~/.brainiac/venv`) is handled correctly — no `pip install -e` against
a wheel install.
...
**RESOLVE THE CHECKOUT BY VERSION, NEVER BY PATH HABIT (2026-08-16).** More
than one clone can exist on a host, and `--engine-src` decides what gets
STAGED into every Cowork workspace. Point it at the older one and the re-stage
ships an old engine while reporting every step `ok`. Measured: `~/brainiac`
sat at **0.20.10** while the real source was at **0.20.13** — the path this
step used to name unconditionally was the stale one.

Pick the checkout whose `pyproject.toml` version matches the engine SSOT, and
verify BEFORE passing it:

```
grep -m1 '^version' <checkout>/pyproject.toml   # must equal `brain --version`
cat <checkout>/dist/COMPAT                      # same version
```
...
`workspace_restage` results), refresh that verified checkout first:

```
git -C "<checkout>" pull --ff-only
```

If no checkout exists at all, clone the public one
(`gh repo clone Autopsias/brainiac "$HOME/brainiac"`) — but when one already
exists, NEVER assume `$HOME/brainiac` is it.

If the pull fails (diverged, auth, network) — report the exact error and
stop; do not run `brain update --engine-src <checkout>` against a checkout
...
brain update --engine-src "<checkout>"          # Cowork workspace registered (Step 1 ran)
```

`<checkout>` is the one Step 1 VERIFIED against the SSOT — never `$HOME/brainiac`
by reflex.

**A version-moving update re-execs itself once** (2026-08-16). Every step after
the venv refresh runs from modules already imported into that process, and pip
replaces files on disk, never modules in memory — so without the re-exec the run
that INSTALLS new staging logic would execute the OLD logic. `BRAIN_UPDATE_REEXECED`
guards against a loop; never set it by hand.
...
<!-- SKILL_VERSION: 0.20.13 (generated by tools/package_clients.py — do not hand-edit) -->
