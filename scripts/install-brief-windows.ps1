...
# Task action: sync (drain + reconcile + publish) then brief.
$scriptBlock = @"
`$env:BRAIN_VAULT = '$VaultPath'
`$env:BRAIN_AUDIT_KEY_PEM = '$auditKey'
`$log = '$LogDir\brief-' + (Get-Date -Format 'yyyy-MM-dd') + '.log'
'=== brain-daily-brief ' + (Get-Date -Format 'yyyy-MM-ddTHH:mm:ssZ') + ' ===' | Out-File -Append -FilePath `$log
& '$BrainExe' sync --publish --json 2>&1 | Out-File -Append -FilePath `$log
'---' | Out-File -Append -FilePath `$log
& '$BrainExe' brief --json 2>&1 | Out-File -Append -FilePath `$log
