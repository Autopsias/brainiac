...
(function () {
  /* Bumped again for the closing-window rescue below. The bump is load-bearing:
   * the early return above keeps an already-installed hook, so a live tab armed
   * with an older version would never register the new handler. A mismatch
   * falls through and keeps `prev.calls` and `prev.rawFetch`, so re-arming an
   * open mail tab costs it nothing. */
  var VERSION = 7;
  ...
  /* THE MUTATION VERBS. Reads are `Get*`, `Find*`, `Subscribe*`, `Expand*` and
   * never match. This list decides only what gets SHARED with the window's
   * siblings, never what is captured — a verb missing here costs a share, not
   * a capture. */
  var MUTATION_RE = /^(Apply|Create|Delete|Discard|Empty|Mark|Move|Send|Set|Update)/;

  /* SHARE A MUTATION THE MOMENT IT IS MADE, not when the window closes.
   * MEASURED 2026-08-28, five times: OWA issues a draft's mutation from a
   * window it then destroys, and `pagehide` does NOT fire on that teardown —
   * the browser closes the target outright. A rescue that waits for a teardown
   * event therefore never runs. Sharing at capture time needs no teardown at
   * all, and a mutation is rare enough that the chatter is negligible.
   *
   * ONE header crosses, and it is the request payload the shape is built from
   * (`cos_mutate_shapes.py::_row_payload` reads it first). `authorization` and
   * every other header stay in the window that made the call. */
  function shareMutation(entry) {
    if (!cap.bc || !entry.action || !MUTATION_RE.test(entry.action)) return;
    var post = entry.headers && entry.headers["x-owa-urlpostdata"];
    if (typeof entry.body !== "string" && !post) return;
    try {
      cap.bc.postMessage({calls: [{
        url: entry.url, method: entry.method, ts: entry.ts,
        action: entry.action, body: entry.body, status: entry.status,
        headers: post ? {"x-owa-urlpostdata": post} : {},
      }]});
    } catch (e) { /* a torn-down channel drops the share, never the page */ }
  }
        ...
        if (!body && input && typeof input.clone === "function" && entry) {
          try {
            input.clone().text().then(function (txt) {
              var had = entry.action;
              entry.body = txt;
              if (!entry.action) entry.action = actionOf(url, entry.headers, txt);
              /* The share at record time saw no action on this one. Now that
               * the body has named it, share it — otherwise a mutation whose
               * verb lives ONLY in an async body is never shared. */
              if (!had && entry.action) shareMutation(entry);
  ...
  /* THE SIBLING CHANNEL. Measured 2026-08-28: this build of OWA opens each
   * draft as its OWN page target, issues the draft's mutation from THAT
   * window, and then destroys it. The buffer died with the window every time,
   * so five owner deletes in a row recorded nothing while read traffic
   * captured perfectly — a mutation lane that could never see a mutation.
   *
   * Two rescues were tried and MEASURED DEAD before this one. Handing the
   * buffer to `window.opener` fails because every one of those windows reports
   * `opener: none` — they are independent top-level windows, not popups.
   * Broadcasting on `pagehide` fails because that event does NOT fire on this
   * teardown; the browser closes the target outright. So the share happens at
   * CAPTURE time (see `shareMutation`), which depends on no teardown at all.
   *
   * A receiver marks what arrives `from_sibling_window`, so a call captured
   * here is never confused with one this window made itself. */
  try {
    if (prev && prev.bc && typeof prev.bc.close === "function") prev.bc.close();
  } catch (e) { /* a closed channel is already what we want */ }
  cap.bc = null;
  if (typeof BroadcastChannel === "function") {
    try {
      cap.bc = new BroadcastChannel("cos-cap-rescue");
      cap.bc.onmessage = function (ev) {
        var rows = ev && ev.data && ev.data.calls;
        if (!rows || !rows.length) return;
        for (var i = 0; i < rows.length; i++) {
          rows[i].from_sibling_window = true;
          cap.calls.push(rows[i]);
        }
        while (cap.calls.length > MAX) { cap.calls.shift(); cap.dropped += 1; }
      };
    } catch (e) { cap.bc = null; }
