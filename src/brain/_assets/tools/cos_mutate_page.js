...
(function () {
  ...
  /* --- BEGIN draft-discard-only primitive ---------------------------------
   * This constant is intentionally NOT in ALLOWED_ACTIONS or REQUEST_TYPE.
   * Generic `send`, `prepare`, `applyOne` and `validate` therefore continue to
   * refuse it. The only caller is `sendDraftDiscard`, after an exact ledgered
   * item has been re-read and `validateDraftDiscard` has matched the separately
   * captured DiscardDraft shape. */
  var DRAFT_DISCARD_ACTION = "DeleteItem";
  var DRAFT_DISCARD_REQUEST = "DeleteItemRequest";
  /* --- END draft-discard-only primitive ----------------------------------- */
  var DRAFT_DISCARD_SHAPE = "DiscardDraft";
  var DRAFT_DISCARD_DELETE_TYPE = "MoveToDeletedItems";
  ...
  /* VERIFY WHAT WE CONTROL, NOT WHAT THE MAILBOX LOOKS LIKE (2026-08-28).
   * A categorize touches exactly ONE chip, so that chip's presence (add) or
   * absence (remove) is the whole question. Comparing the full category SET
   * instead made an unrelated label a failure: on 2026-08-28 both runs of the
   * day stopped dead because Outlook added `Action` to a `Held · draft` thread
   * in the same moment — the run's own chip was there and correct, and it
   * still scored `verified-failed` and killed the remaining plan plus the
   * whole second batch. Callers keep the set compare for the SILENT NO-OP
   * check, which is a different question and still needs it. */
  function chipHeld(observed, mode, chip) {
    if (!observed) { return false; }
    var present = observed.indexOf(chip) !== -1;
    return mode === "remove" ? !present : present;
  }

  /* Labels on the thread that this run did not ask for. Reported, never
   * failed on — the owner cannot see a side effect nothing records. */
  function unrequestedChips(observed, after) {
    return (observed || []).filter(function (c) {
      return (after || []).indexOf(c) === -1;
    });
  }
  ...
  /* The deletion primitive's separate, draft-only gate. `validate()` above
   * still rejects DeleteItem because it is absent from ALLOWED_ACTIONS. This
   * validator accepts exactly one captured shape, one exact re-read draft id,
   * and the recoverable MoveToDeletedItems disposition. */
  function validateDraftDiscard(body, ctx) {
    ctx = ctx || {};
    if (!body || typeof body !== "object" || !body.Header) {
      return "draft discard carries no captured request Header to replay";
    }
    var reqType = String(get(body, "Body.__type") || "");
    if (reqType.indexOf(DRAFT_DISCARD_REQUEST) !== 0) {
      return "draft discard request type is " + JSON.stringify(reqType)
        + ", not the dedicated captured request type";
    }
    var shape = (ctx.shapes || {})[DRAFT_DISCARD_SHAPE];
    if (!shape || !shape.fingerprint) {
      return "no APPROVED captured draft-discard shape is on file";
    }
    var got = fingerprint(body);
    if (got !== shape.fingerprint) {
      return "draft-discard payload fingerprint " + got
        + " does not match the approved shape " + shape.fingerprint;
    }
    if (get(body, "Body.DeleteType") !== DRAFT_DISCARD_DELETE_TYPE) {
      return "draft discard DeleteType must be " + DRAFT_DISCARD_DELETE_TYPE
        + "; a hard delete is never admitted";
    }
    var ids = get(body, "Body.ItemIds");
    if (!Array.isArray(ids) || ids.length !== 1 || !ids[0] || !ids[0].Id) {
      return "draft discard must carry exactly one item id";
    }
    if (!ctx.item_id || ids[0].Id !== ctx.item_id) {
      return "draft discard item id is not the exact id just re-read in Drafts";
    }
    if (!ctx.change_key || ids[0].ChangeKey !== ctx.change_key) {
      return "draft discard ChangeKey is not the value just re-read from the item";
    }
    if (get(body, "Body.MessageDisposition") !== undefined
        || get(body, "Body.Items") !== undefined
        || get(body, "Body.ConversationActions") !== undefined) {
      return "draft discard carries a send/create/conversation payload beside its item id";
    }
    var values = stringValues(body);
    for (var i = 0; i < values.length; i++) {
      /* DeleteItem's own cancellation field uses `SendToNone` to assert that
       * no meeting cancellation is sent. It is accepted only at that exact
       * path; the same string anywhere else remains a refused disposition. */
      var noCancellation = values[i].path === "Body.SendMeetingCancellations"
        && values[i].value === "SendToNone";
      if (BANNED_DISPOSITIONS.indexOf(values[i].value) !== -1
          && !noCancellation) {
        return "draft discard carries a refused sending disposition";
      }
      for (var b = 0; b < BANNED_REQUESTS.length; b++) {
        var banned = BANNED_REQUESTS[b];
        if (banned === DRAFT_DISCARD_ACTION) continue;
        if (values[i].value.indexOf(banned + "Request") === 0
            || values[i].value.indexOf(banned + "Type") === 0) {
          return "draft discard carries a second refused request type";
        }
      }
    }
    return null;
  ...
  function buildDraftDiscard(shapes, header, itemId, changeKey) {
    var body = requireShape(shapes, DRAFT_DISCARD_SHAPE);
    body.Header = clone(header);
    var ids = need(body, "Body.ItemIds", DRAFT_DISCARD_SHAPE);
    if (!Array.isArray(ids) || ids.length !== 1) {
      throw new Error("the approved draft-discard shape does not carry exactly "
        + "one ItemId — recapture a single disposable-draft delete");
    }
    var target = clone(ids[0]);
    target.Id = itemId;
    target.ChangeKey = changeKey;
    body.Body.ItemIds = [target];
    return body;
  }
  ...
  function sendDraftDiscard(body, opts) {
    opts = opts || {};
    if (!armed) {
      var last = lastTransition();
      var why = "draft-discard dispatch is DISARMED ("
        + (last ? "HTTP " + last.kind.replace("http-", "") + " seen at " + last.at
                : "no transition recorded") + ")";
      rejected.push({action: DRAFT_DISCARD_SHAPE, at: now(), reason: why,
                     conversation_id: opts.conversation_id || null});
      return Promise.reject(new Error(why));
    }
    var reason = validateDraftDiscard(body, opts.ctx || {});
    if (reason) {
      rejected.push({action: DRAFT_DISCARD_SHAPE, at: now(), reason: reason,
                     conversation_id: opts.conversation_id || null,
                     fingerprint: fingerprint(body)});
      return Promise.reject(new Error("REFUSED BEFORE DISPATCH: " + reason));
    }
    return dispatch(DRAFT_DISCARD_ACTION, body, opts, true);
  }
  ...
  function _getItemShaped(itemId, baseShape, bodyType) {
    ...
    return send("GetItem", body).then(function (r) {
      ...
      return {
        ...
        /* Positive evidence of a QUEUED send. `isDraft: true` is already
         * required upstream, so on a confirmed draft DateTimeSent is only an
         * OWA save-time artifact (OWA stamps DateTimeSent = DateTimeCreated on
         * unsent drafts); IsSubmitted is the one field that flags a draft the
         * user queued to send. A genuinely sent message has isDraft:false and
         * lives in Sent Items, so it can never reach this discard-only check. */
        sentEvidence: item.IsSubmitted === true,
        dateTimeSent: item.DateTimeSent || null,
  ...
  function machineMarkers(text) {
    return String(text || "").match(/\[cos:[^\]\r\n]{1,300}\]/g) || [];
  }

  /* Re-read the exact saved item immediately before the delete call. Folder
   * membership is proved by a complete Drafts enumeration; ownership is the
   * conjunction of exact item id, conversation id and one unambiguous run
   * signature. A matching signature under a rotated id is reported, never
   * substituted. */
  function inspectDraftCandidate(candidate) {
    candidate = candidate || {};
    var sig = String(candidate.signature || "");
    var cid = String(candidate.conversation_id || "");
    var itemId = String(candidate.item_id || "");
    var runId = String(candidate.run_id || "");
    if (!sig || !cid || !itemId || !runId
        || sig.indexOf("[cos:" + runId + ":") !== 0 || sig.slice(-1) !== "]") {
      return Promise.resolve({eligible: false,
        outcome: "missing-or-malformed-ledger-identity",
        changekey_refetched_at: now(),
        receipts: {is_draft: false, signature_present: false,
          send_attempted: false, item_id_matches: false,
          conversation_matches: false, signature_matches: false,
          signature_unambiguous: false, sent_evidence_absent: false,
          request_not_send: true, source_folder: DRAFT_FOLDER,
          absent_from_drafts: false, drafts_enumeration_complete: false}});
    }
    return enumerate(DRAFT_FOLDER).then(function (drafts) {
      var sameConversation = drafts.items.filter(function (i) {
        return i.convId === cid;
      });
      return Promise.all(sameConversation.map(function (i) {
        return getItem(i.itemId);
      })).then(function (conversationItems) {
        return getItem(itemId).then(function (exact) {
          var exactListed = drafts.items.some(function (i) {
            return i.itemId === itemId;
          });
          var text = String(exact.text || "");
          var exactOccurrences = occurrences(text, sig);
          var markers = machineMarkers(text);
          var matchingIds = [];
          conversationItems.forEach(function (full, i) {
            if (full.ok && occurrences(String(full.text || ""), sig) > 0) {
              matchingIds.push(sameConversation[i].itemId);
            }
          });
          var itemMatches = exact.ok && exact.itemId === itemId;
          var conversationMatches = exact.ok && exact.convId === cid;
          var signed = exactOccurrences === 1;
          var unambiguous = signed && markers.length === 1
            && markers[0] === sig && matchingIds.length === 1
            && matchingIds[0] === itemId;
          var noSentEvidence = exact.ok && exact.sentEvidence === false;
          var eligible = drafts.complete && exactListed && itemMatches
            && exact.isDraft === true && conversationMatches && unambiguous
            && noSentEvidence && !!exact.changeKey;
          var outcome = "eligible-signed-draft";
          if (!drafts.complete) outcome = "drafts-enumeration-incomplete";
          else if (!exactListed && matchingIds.length) outcome = "draft-item-id-rotated";
          else if (!exactListed) outcome = "draft-item-id-not-in-drafts";
          else if (!itemMatches) outcome = "draft-item-id-mismatch";
          else if (exact.isDraft !== true) outcome = "current-item-is-not-a-draft";
          else if (!conversationMatches) outcome = "conversation-id-mismatch";
          else if (exact.sentEvidence === true) outcome = "current-item-has-send-evidence";
          else if (!signed) outcome = "signature-missing-or-repeated";
          else if (!unambiguous) outcome = "signature-identity-ambiguous";
          else if (!exact.changeKey) outcome = "draft-change-key-missing";
          return {
            eligible: eligible, outcome: outcome,
            item_id: itemId, change_key: exact.changeKey || null,
            changekey_refetched_at: now(),
            receipts: {
              is_draft: exact.isDraft === true,
              signature_present: exactOccurrences > 0,
              send_attempted: false,
              item_id_matches: itemMatches,
              conversation_matches: conversationMatches,
              signature_matches: signed,
              signature_unambiguous: unambiguous,
              sent_evidence_absent: noSentEvidence,
              request_not_send: true,
              source_folder: DRAFT_FOLDER,
              absent_from_drafts: false,
              drafts_enumeration_complete: drafts.complete,
            },
          };
        });
      });
  ...
  function discardDraft(candidate) {
    return inspectDraftCandidate(candidate).then(function (pre) {
      if (!pre.eligible) {
        return {verb: "discard-draft",
                conversation_id: candidate.conversation_id,
                state: "aborted-not-applied", dispatched: false,
                outcome: pre.outcome, verification: "draft-discard-refused",
                changekey_refetched_at: pre.changekey_refetched_at,
                receipts: pre.receipts};
      }
      var body = buildDraftDiscard(cfg.shapes, cfg.header,
                                   pre.item_id, pre.change_key);
      return sendDraftDiscard(body, {
        conversation_id: candidate.conversation_id,
        ctx: {shapes: cfg.shapes, item_id: pre.item_id,
              change_key: pre.change_key},
      }).then(function (r) {
        var msg = firstItem(r);
        ...
        if (code !== "NoError") {
          return {verb: "discard-draft",
                  conversation_id: candidate.conversation_id,
                  state: "sent", dispatched: true,
                  response_code: code, outcome: "delete-response-not-noerror",
                  verification: "verified-failed",
                  changekey_refetched_at: pre.changekey_refetched_at,
                  receipts: pre.receipts};
        }
        /* Two complete scans, agreeing on the folder size: the same
         * concurrent-change belt used by the ordinary not-found path. */
        return enumerate(DRAFT_FOLDER).then(function (one) {
          return enumerate(DRAFT_FOLDER).then(function (two) {
            var present = one.items.concat(two.items).some(function (i) {
              return i.itemId === pre.item_id;
            });
            var complete = one.complete && two.complete
              && one.items_seen === two.items_seen;
            var absent = !present && complete;
            var receipts = Object.assign({}, pre.receipts, {
              absent_from_drafts: absent,
              drafts_enumeration_complete: complete,
              request_not_send: true,
            });
            return {verb: "discard-draft",
                    conversation_id: candidate.conversation_id,
                    state: absent ? "reconciled" : "confirmed",
                    dispatched: true, response_code: code,
                    outcome: absent ? "draft-absent-from-drafts"
                                    : "post-delete-draft-still-present-or-unreadable",
                    verification: absent ? "verified-draft-discarded"
                                         : "verified-failed",
                    changekey_refetched_at: pre.changekey_refetched_at,
                    receipts: receipts};
          });
  ...
  function verifyOne(m, p, r) {
    ...
    if (m.verb === "categorize") {
      ...
      return getItem(p.t.itemId).then(function (re) {
        ...
        var ok = re.ok && chipHeld(re.categories, m.mode, m.chip);
        return {
          verb: "categorize", conversation_id: m.conversation_id,
          state: ok ? "reconciled" : "confirmed", dispatched: true,
          before_image: p.before, after: p.after, observed_after: re.categories,
          unrequested_categories: unrequestedChips(re.categories, p.after),
          changekey_refetched_at: p.t.changekey_refetched_at,
  ...
  function exportShapes(capture) {
    ...
    /* Separately captured, separately keyed: it never joins ALLOWED_ACTIONS,
     * and therefore cannot become a generic mutation capability by export. */
    jobs.push({action: DRAFT_DISCARD_ACTION, conv: null,
               key: DRAFT_DISCARD_SHAPE, draftDiscard: true});
    jobs.forEach(function (job) {
      ...
      for (var i = 0; i < cap.calls.length; i++) {
        ...
        if (job.draftDiscard
            && (String(body.Body.__type || "").indexOf(DRAFT_DISCARD_REQUEST) !== 0
                || body.Body.DeleteType !== DRAFT_DISCARD_DELETE_TYPE)) continue;
  ...
  var api = {
    ...
    validate: validate,
    validateDraftDiscard: validateDraftDiscard,
    buildMove: buildMove,
    buildConversationMove: buildConversationMove,
    buildConversationCategorize: buildConversationCategorize,
    ...
    buildDraftDiscard: buildDraftDiscard,
    ...
    prepare: prepare,
    applyOne: applyOne,
    inspectDraftCandidate: inspectDraftCandidate,
  ...
  function bootBridge() {
    ...
    function handle(op, args) {
      ...
      if (op === "discard-draft") return discardDraft(args.candidate || {});
