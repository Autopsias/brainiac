...
(strongest) an RFC-3161 timestamp token from a third-party timestamp authority. Anchoring into
...
Ported FROM SCRATCH from the Example Corp vault's ``_chain_anchor.py`` *pattern* — no
...
def anchor(chain_log: Path, anchor_dir: Path, *, timestamp_token: bytes | None = None) -> dict[str, Any]:
    ...
    rec = {
        ...
        "entry_count": count,
        "head": head,
        "timestamp_token": bool(timestamp_token),
    }
    log = anchor_dir / ANCHOR_LOG_NAME
    with log.open("a", encoding="utf-8") as f:
        f.write(json.dumps(rec, sort_keys=True, separators=(",", ":")) + "\n")
    if timestamp_token:  # pragma: no cover - external timestamp-authority round-trip
        (anchor_dir / f"head-{count}.tsr").write_bytes(timestamp_token)
