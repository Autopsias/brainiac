...
def build_parser() -> argparse.ArgumentParser:
    ...
    sp = sub.add_parser(
        "curate",
        help="curation fold: refresh-index + unclassified-notes lint (host); "
             "orphan/stale-link/contradiction/callout lint stay vault-overlay (no brain equivalent)",
    )
    ...
    sp = sub.add_parser(
        ...
        help="the umbrella: THE single sanctioned host task (brain-nightly) — "
             "sync --publish + brief, plus date-gated health/integrity/digest branches",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if cmd == "curate":
        ...
        if args.json:
            _emit({**res, "unclassified_notes": surfaced, "egress": report,
