...
search and grep HIDE versions a supersede chain retired (`is_latest_version:
false`) by default — `--include-retired` brings them back for a "previous
version" question. Every surfaced hit still carries `is_latest_version`, so an
agent can prefer the current claim without a second round-trip.
