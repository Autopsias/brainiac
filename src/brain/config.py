...
# Host-private / per-surface path resolution lives in config_hostpaths.py
# since the 2026-08-16 size ratchet; re-exported so every `brain.config.<name>`
# caller (and the tests that monkeypatch `config._app_data_base`, which the
# sibling resolves through THIS namespace at call time) is unchanged.
from .config_hostpaths import (  # noqa: E402,F401  (facade re-export)
    _LOCK_FALLBACK_REPORTED as _LOCK_FALLBACK_REPORTED,
    _REGISTRY_HOME_ENV as _REGISTRY_HOME_ENV,
    _declared_workspace_roots as _declared_workspace_roots,
    _ensure as _ensure,
    _health_history_root as _health_history_root,
    HostPathUnsafe as HostPathUnsafe,
    WORKSPACE_ROOT_ENV as WORKSPACE_ROOT_ENV,
    anchor_dir as anchor_dir,
    audit_drift_dispositions_dir as audit_drift_dispositions_dir,
    audit_drift_dispositions_path as audit_drift_dispositions_path,
    cos_ops_dir as cos_ops_dir,
    cos_run_records_dir as cos_run_records_dir,
    graph_build_failed_marker_path as graph_build_failed_marker_path,
    graph_dir as graph_dir,
    graph_json_path as graph_json_path,
    graph_manifest_path as graph_manifest_path,
    health_archive_dir as health_archive_dir,
    health_history_lock_path as health_history_lock_path,
    health_history_path as health_history_path,
    health_sparse_path as health_sparse_path,
    host_lock_dir as host_lock_dir,
    host_private_base as host_private_base,
    proven_off_mount as proven_off_mount,
    supersede_journal_path as supersede_journal_path,
    vm_visible_roots as vm_visible_roots,
    warn_if_lock_dir_fallback as warn_if_lock_dir_fallback,
    writer_lock_path as writer_lock_path,
)
