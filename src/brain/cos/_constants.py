...
#: THE FLOOR UNDER THAT PERCENTAGE, AND IT IS AN OVERHEAD, NOT A PERCENTAGE.
#: `approx_size_bytes` is the size the mail app reports for an attachment, and
#: it consistently OVERSHOOTS the bytes the fetch writes by a per-message
#: CONSTANT — measured over 26 claimed attachments on a live vault 2026-08-23:
#: 422 bytes on all three images of one message, 378 on both files of another,
#: and roughly 8.3 KB on the largest. Median absolute error 0.5%, max 24%. A
#: proportional tolerance therefore mis-sizes the guard at both ends: it is far
#: too wide for a 1 MB file and too narrow for a 50 KB one. At 4096 the floor
#: covered the two worst PERCENTAGE errors (small images) and still refused an
#: intact 43,317-byte document whose line claimed 51,546 — an 8,229-byte
#: overhead against a 5,154-byte allowance. 16 KB clears every overhead
#: observed with roughly twice the margin; above ~164 KB the 10% rule takes
#: over again and this floor never applies.
INGEST_SWEEP_SIZE_FLOOR = 16 * 1024
