...
plan's consensus hardening requires a real control. We choose option (a):
    ...
    Project a workspace that physically EXCLUDES Restricted/Secret (and anything
...
def project_workspace(vault: Path, dest: Path, max_tier: str = cls.DEFAULT_MAX_TIER) -> ProjectionResult:
    ...
    Unlabelled/unrecognised notes are default-denied (treated as Secret) and
    excluded unless ``max_tier == "Secret"``. ``dest`` is recreated from scratch
