...
class TestEnforce:
    ...
    def test_classification_defaults_to_internal_not_mnpi(self):
...
# ---------------------------------------------------------------------------
# C-1 / C-2 — path-traversal guards (S01 hardening regression tests)
# ---------------------------------------------------------------------------

class TestTraversalGuards:
    """An untrusted id must never escape capture-inbox/ (C-1) or trick the
    host into signing a write outside the intended subtree (C-2)."""

    # -- C-1: draft_capture ------------------------------------------------

    def test_draft_capture_rejects_traversal_ident(self, tmp_path):
        vault = _mini_vault(tmp_path)
        core = BrainCore(vault=vault, index=BrainIndex(
            db_path=tmp_path / "i.sqlite", backend=BruteForceBackend(),
            embedder=HashEmbedder()), role="host")
        victim = vault / "brain" / "resources" / "victim.md"
        victim.write_text("original", encoding="utf-8")
        with pytest.raises(ValueError):
            core.draft_capture("body", ident="../../brain/resources/victim")
        assert victim.read_text(encoding="utf-8") == "original"
        inbox = config.capture_inbox_dir(vault)
        assert not (inbox.is_dir() and list(inbox.glob("*.md"))), \
            "no draft may be staged for a refused id"

    def test_draft_capture_rejects_traversal_id_from_yaml(self, tmp_path):
        vault = _mini_vault(tmp_path)
        core = BrainCore(vault=vault, index=BrainIndex(
            db_path=tmp_path / "i.sqlite", backend=BruteForceBackend(),
            embedder=HashEmbedder()), role="host")
        content = "---\nid: ../../brain/index\ntype: note\n---\n\nBooby-trapped.\n"
        with pytest.raises(ValueError):
            core.draft_capture(content)

    @pytest.mark.parametrize("bad_id", [
        "a/b", "/etc/passwd", "..", "a\\b", ".hidden", "a b",
        "x\x00y", "．．/x",          # NUL; fullwidth-dot variant
        "a⁄b",                            # fraction slash
    ])
    def test_draft_capture_rejects_non_slug_ids(self, tmp_path, bad_id):
        vault = _mini_vault(tmp_path)
        core = BrainCore(vault=vault, index=BrainIndex(
            db_path=tmp_path / "i.sqlite", backend=BruteForceBackend(),
            embedder=HashEmbedder()), role="host")
        with pytest.raises(ValueError):
            core.draft_capture("body", ident=bad_id)

    def test_draft_capture_accepts_new_nonexistent_slug_target(self, tmp_path):
        """Valid slug for a file that does not exist yet (Path.resolve on a
        nonexistent path) must still stage fine."""
        vault = _mini_vault(tmp_path)
        core = BrainCore(vault=vault, index=BrainIndex(
            db_path=tmp_path / "i.sqlite", backend=BruteForceBackend(),
            embedder=HashEmbedder()), role="host")
        res = core.draft_capture("body", ident="2026-07-04-fresh-note")
        assert Path(res["draft"]).exists()
        assert res["id"] == "2026-07-04-fresh-note"

    def test_draft_capture_refuses_symlink_escaping_inbox(self, tmp_path):
        """A symlink planted inside the inbox pointing outside the vault must
        ...
        os.symlink(outside, inbox / "sneaky.md")
        with pytest.raises(ValueError):
            core.draft_capture("body", ident="sneaky")
        assert not outside.exists(), "write must not follow the symlink out"

    def test_cli_draft_capture_bad_id_exits_3(self, tmp_path, monkeypatch):
        vault = _mini_vault(tmp_path)
        monkeypatch.setenv("BRAIN_VAULT", str(vault))
        ret = cli.main(["draft-capture", "--content", "x",
                        "--id", "../../brain/index", "--json"])
        assert ret == 3, "CLI must fail closed (exit 3) on a traversal id"

    # -- C-2: drain_drafts / write_note -------------------------------------

    def test_drain_skips_traversal_id_draft_and_leaves_it(self, tmp_path, audit_key_env):
        vault = _mini_vault(tmp_path)
        core = _host_core(tmp_path, vault)
        victim = vault / "brain" / "resources" / "real-note.md"
        victim.write_text("original", encoding="utf-8")
        inbox = config.capture_inbox_dir(vault)
        inbox.mkdir(parents=True, exist_ok=True)
        evil = inbox / "evil.md"
        evil.write_text(
            "---\nid: ../../brain/resources/real-note\ntype: source\n"
            "classification: Internal\ncreated: 2026-07-04\nstatus: draft\n---\n\npwn\n",
            encoding="utf-8",
        )
        ...
        assert any("unsafe-id" in r for r in reasons)
        assert evil.exists(), "refused draft must be LEFT in place, not deleted"
        assert victim.read_text(encoding="utf-8") == "original"
        # Nothing was signed for the refused draft.
        chain = core.verify_audit()
        assert chain["entries_checked"] == 0

    def test_write_note_rejects_vault_escape(self, tmp_path, audit_key_env):
        vault = _mini_vault(tmp_path)
        core = _host_core(tmp_path, vault)
        with pytest.raises(ValueError):
            core.write_note("../outside.md", "x")
        assert not (tmp_path / "outside.md").exists()

    def test_write_note_enforces_specific_subtree(self, tmp_path, audit_key_env):
        vault = _mini_vault(tmp_path)
        core = _host_core(tmp_path, vault)
        # In-vault but outside the intended subtree: refused.
        with pytest.raises(ValueError):
            core.write_note("brain/resources/x.md", "x", subtree="raw")
        with pytest.raises(ValueError):
            core.write_note("raw/../brain/resources/x.md", "x", subtree="raw")
        assert not (vault / "brain" / "resources" / "x.md").exists()
        # Refusal happens BEFORE signing — audit chain stays empty.
        assert core.verify_audit()["entries_checked"] == 0
        # The honest path still works.
        res = core.write_note("raw/good.md", "---\nid: good\n---\nok", subtree="raw")
        assert Path(res["written"]).exists()

    def test_write_note_refuses_symlink_inside_vault_pointing_outside(
        ...
        outside = tmp_path / "smuggled.md"
        os.symlink(outside, vault / "raw" / "link.md")
        with pytest.raises(ValueError):
            core.write_note("raw/link.md", "x", subtree="raw")
        assert not outside.exists()

    def test_drain_skips_symlinked_subtree_via_write_note_guard(
        self, tmp_path, audit_key_env
    ):
        """Valid slug id, but vault/raw is a symlink out of the vault — the
        resolved-path containment must refuse and the drain must skip."""
        import os
        vault = _mini_vault(tmp_path)
        core = _host_core(tmp_path, vault)
        outside = tmp_path / "outside-raw"
        outside.mkdir()
        os.symlink(outside, vault / "raw")
        inbox = config.capture_inbox_dir(vault)
        inbox.mkdir(parents=True, exist_ok=True)
        (inbox / "d.md").write_text(
            "---\nid: legit-source\ntype: source\nclassification: Internal\n"
            "created: 2026-07-04\nstatus: draft\n---\n\nbody\n",
            encoding="utf-8",
        )
        ...
        assert any("unsafe-path" in s["reason"] for s in res["details"]["skipped"])
        assert not list(outside.glob("*.md")), "nothing may land outside the vault"

    def test_safe_slug_accepts_existing_corpus_ids(self):
        from brain.notes import safe_slug
        for ok in ("arctic-embed-choice", "2026-06-27-arctic-vs-e5-benchmark",
                   "draft-ab12cd34ef56", "raw-readme", "index", "v1.2-notes"):
            assert safe_slug(ok) == ok
