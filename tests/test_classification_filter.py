...
def test_tier_ordering():
    assert cls.rank("Public") < cls.rank("Internal") < cls.rank("Confidential") \
        < cls.rank("Restricted") < cls.rank("MNPI")


@pytest.mark.parametrize("value", [None, "", "bogus", "secret", 42, "internal "])
def test_unrecognised_is_default_denied_as_mnpi(value):
    ...
    if isinstance(value, str) and value.strip() in cls.RANK:
        ...
        assert cls.normalize(value) == "MNPI"
        assert cls.rank(value) == cls.RANK["MNPI"]
...
def test_default_max_tier_is_conservative():
    ...
    assert not flt.allows("MNPI")
    assert not flt.allows(None)  # unlabelled -> MNPI -> denied
...
def test_only_mnpi_cap_surfaces_unlabelled():
    flt = cls.ClassificationFilter(max_tier="MNPI")
    assert flt.allows(None)
    assert flt.allows("MNPI")
...
def test_casing_mismatch_is_detected_but_still_fail_closed():
    ...
    # strict matching: lowercase 'internal' is treated as MNPI (not surfaced at default)
    flt = cls.ClassificationFilter(max_tier="Internal")
