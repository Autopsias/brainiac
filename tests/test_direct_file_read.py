...
Proves, on each supported surface, the security boundary:
  1. The cooperative `brain` path WITHHOLDS restricted/Secret (egress decision).
  ...
  3. The REAL control — a projected workspace (brain.projection) — physically
     EXCLUDES restricted/Secret, so a direct file read on THAT surface cannot
...
SENSITIVE = ("restricted-deal", "secret-merger", "confidential-pricing", "unlabelled")
...
def test_cli_path_withholds_but_files_on_disk_expose(sample_vault, tmp_path):
    ...
    assert "secret Atlas counterparty" in raw          # restricted leaked on disk
    assert "insider nonpublic merger" in raw             # Secret leaked on disk
    # in-process core sees them too (documents the bypass)
    assert "restricted-deal" in in_process_ids or "secret-merger" in in_process_ids
...
def test_projection_excludes_sensitive_on_disk(sample_vault, tmp_path):
    ...
    assert "secret Atlas counterparty" not in projected
    assert "insider nonpublic merger" not in projected
...
def test_projection_recreates_clean_each_run(sample_vault, tmp_path):
    ...
    project_workspace(sample_vault, dest, max_tier="Secret")
    assert "secret Atlas counterparty" in _read_all_text(dest)
    project_workspace(sample_vault, dest, max_tier="Internal")
    # re-projection must not retain the previously-copied sensitive file
    assert "secret Atlas counterparty" not in _read_all_text(dest)
