...
def linked_vault(tmp_path, monkeypatch):
    ...
    files = {
        "brain/resources/hub.md": _note(
            "hub", "Hub", "Internal",
            "Meridian deal hub linking everything",
            links=["public-fact", "internal-fact", "confi-pricing", "restricted-deal", "mnpi-secret"]),
        "brain/resources/public-fact.md": _note(
            "public-fact", "Public Fact", "Public", "Meridian public deal fact"),
        "brain/resources/internal-fact.md": _note(
            "internal-fact", "Internal Fact", "Internal", "Meridian internal deal fact"),
        "brain/resources/confi-pricing.md": _note(
            "confi-pricing", "Confidential Pricing", "Confidential",
            "Meridian confidential deal pricing"),
        "brain/projects/restricted-deal.md": _note(
            "restricted-deal", "Restricted Deal", "Restricted",
            "Meridian restricted deal counterparty secret"),
        "brain/projects/mnpi-secret.md": _note(
            "mnpi-secret", "MNPI Secret", "MNPI",
            "Meridian mnpi deal material nonpublic"),
        # unlabelled => default-deny (MNPI)
        "brain/resources/unlabelled.md": _note(
            "unlabelled", "Unlabelled", "", "Meridian deal unlabelled note"),
    }
...
SENSITIVE_TOKENS = ("counterparty secret", "material nonpublic")
...
def test_subcommand_enumeration_is_complete():
    ...
    # curate/integrity/promote-scan (CUT-03 maintenance rituals) are covered by
    # their own dedicated egress tests in tests/test_maintenance.py; graphify
    # (GRF-01) is covered by tests/test_graphify.py — listed here only so this
    # exhaustiveness check stays accurate.
    exercised = {"search", "hybrid-search", "grep", "bases-query",
                 "graph-expand", "get", "read", "recent",
                 "curate", "integrity", "promote-scan",
                 "brief", "digest", "graphify"}
...
def test_search_aliases_gate(linked_vault, cmd):
    code, res = _json([cmd, "Meridian deal", "-k", "20", "--json"])
    assert code == 0
    ids = {r["id"] for r in res["results"]}
    assert "restricted-deal" not in ids and "mnpi-secret" not in ids
    assert "unlabelled" not in ids
    assert res["egress"]["withheld"] >= 3
    # elevation surfaces them (the explicit human gate)
    _, hi = _json([cmd, "Meridian deal", "-k", "20", "--max-tier", "MNPI", "--json"])
    hi_ids = {r["id"] for r in hi["results"]}
    assert {"restricted-deal", "mnpi-secret", "unlabelled"} <= hi_ids
...
def test_search_rerank_path_still_gates(linked_vault):
    ...
    code, res = _json(["search", "Meridian deal", "-k", "20", "--rerank", "--json"])
    assert code == 0
    ids = {r["id"] for r in res["results"]}
    assert not ({"restricted-deal", "mnpi-secret", "unlabelled"} & ids)
    assert res["egress"]["withheld"] >= 3


def test_grep_gates(linked_vault):
    code, res = _json(["grep", "Meridian", "-k", "50", "--json"])
    assert code == 0
    ids = {r["id"] for r in res["results"]}
    assert not ({"restricted-deal", "mnpi-secret", "unlabelled"} & ids)
    assert res["egress"]["withheld"] >= 3
...
@pytest.mark.parametrize("cmd", ["brief", "digest"])
def test_brief_digest_gate_on_host(linked_vault, cmd):
    argv = [cmd, "-n", "50", "--json"] if cmd == "brief" else [cmd, "--days", "3650", "--json"]
    code, res = _json(argv)
    assert code == 0
    notes = res["recent"] if cmd == "brief" else res["notes"]
    ids = {n["id"] for n in notes}
    assert not ({"restricted-deal", "mnpi-secret", "unlabelled"} & ids)
    assert res["egress"]["withheld"] >= 3
    # elevation surfaces them (the explicit human gate)
    hi_argv = argv[:-1] + ["--max-tier", "MNPI", "--json"]
    _, hi = _json(hi_argv)
    hi_notes = hi["recent"] if cmd == "brief" else hi["notes"]
    hi_ids = {n["id"] for n in hi_notes}
    assert {"restricted-deal", "mnpi-secret", "unlabelled"} <= hi_ids


@pytest.mark.parametrize("cmd", ["brief", "digest"])
def test_brief_digest_gate_on_vm(linked_vault, cmd):
    # publish a snapshot so the VM leg has something to read
    code, _ = _run(["snapshot"])
    assert code == 0

    tail = ["-n", "50", "--json"] if cmd == "brief" else ["--days", "3650", "--json"]
    argv = ["--role", "vm", cmd] + tail
    code, res = _json(argv)
    assert code == 0
    notes = res["recent"] if cmd == "brief" else res["notes"]
    ids = {n["id"] for n in notes}
    assert not ({"restricted-deal", "mnpi-secret", "unlabelled"} & ids)
    assert res["egress"]["withheld"] >= 3
    # elevation surfaces them even on the VM leg (read-only, still the human gate)
    hi_argv = ["--role", "vm", cmd] + tail[:-1] + ["--max-tier", "MNPI", "--json"]
    _, hi = _json(hi_argv)
    hi_notes = hi["recent"] if cmd == "brief" else hi["notes"]
    hi_ids = {n["id"] for n in hi_notes}
    assert {"restricted-deal", "mnpi-secret", "unlabelled"} <= hi_ids
...
def test_no_sensitive_token_leaks_via_any_default_path(linked_vault):
    ...
    for argv in (["search", "Meridian deal", "-k", "20", "--json"],
                 ["grep", "Meridian", "-k", "50", "--json"],
