...
def test_flip_on_round_trips(monkeypatch, enc_key_env):
    ...
    token = enc.encrypt_bytes(b"insider nonpublic")
    assert token.startswith(enc.MAGIC)
    assert b"insider nonpublic" not in token  # actually encrypted
    assert enc.decrypt_bytes(token) == b"insider nonpublic"
