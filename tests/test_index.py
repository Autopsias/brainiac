...
def test_grep_rejects_overlong_pattern(sample_vault, tmp_path):
    ...
    with pytest.raises(GrepPatternError):
        idx.grep(too_long, regex=False)
    # a pattern right at the cap is still accepted (boundary, not off-by-one).
    # Non-regex mode so the assertion holds regardless of whether the optional
    # `regex` engine (M-1's --regex gate) is installed in this environment.
    at_cap = "a" * MAX_GREP_PATTERN_LEN
    idx.grep(at_cap, regex=False)  # must not raise
