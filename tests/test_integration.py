...
def test_full_loop_vm_role_system_invariants_hold_throughout(tmp_path, audit_key_env, monkeypatch, capsys):
    """S05 T-1 area 7 — the plan verifies individual patches (VM never resolves
    a key, VM index is read-only, drain promotes+signs, egress gates) in
    isolation; this drives the SAME full loop through the CLI as ONE system
    test and asserts every VM-leg invariant holds at every step: no WAL/SHM
    ever appears next to the VM's snapshot, the signing key is never resolved
    on the VM leg, and the classification egress gate is applied on the VM's
    read after commit (an MNPI draft stays withheld end to end)."""
    import sqlite3
    import brain.audit as audit_mod
    ...
    key_calls = {"n": 0}
    real_resolve = audit_mod.resolve_signing_key
    monkeypatch.setattr(
        audit_mod, "resolve_signing_key",
        lambda *a, **k: (key_calls.__setitem__("n", key_calls["n"] + 1), real_resolve(*a, **k))[1],
    )

    # 1. VM stages two drafts: one Internal (surfaced), one MNPI (withheld).
    assert cli.main(["--role", "vm", "draft-capture", "--id", "vm-open",
                     "--content", "---\nid: vm-open\ntitle: Open\ntype: note\n"
                     "classification: Internal\ncreated: 2026-06-27\nupdated: 2026-06-27\n"
                     "---\n\nVM open note.\n", "--json"]) == 0
    capsys.readouterr()
    assert cli.main(["--role", "vm", "draft-capture", "--id", "vm-secret",
                     "--content", "---\nid: vm-secret\ntitle: Secret\ntype: note\n"
                     "classification: MNPI\ncreated: 2026-06-27\nupdated: 2026-06-27\n"
                     "---\n\nVM secret note.\n", "--json"]) == 0
    capsys.readouterr()
    assert key_calls["n"] == 0  # draft_capture never touched signing

    # 2. HOST rebuilds, drains + signs + indexes, and republishes.
    assert cli.main(["rebuild"]) == 0
    capsys.readouterr()
    assert cli.main(["sync", "--json"]) == 0
    sync_out = json.loads(capsys.readouterr().out)
    assert sync_out["drain"]["promoted"] == 2
    assert key_calls["n"] >= 1  # the HOST resolved the key, not the VM
    assert cli.main(["snapshot"]) == 0
    capsys.readouterr()

    # 3. VM reads the published snapshot: no WAL/SHM sidecar, gate applied.
    snap_dir = config.snapshot_dir(v)
    snap_db = snap_dir / SNAPSHOT_DB
    assert not (snap_dir / (SNAPSHOT_DB + "-wal")).exists()
    assert not (snap_dir / (SNAPSHOT_DB + "-shm")).exists()
    key_calls_before_vm_read = key_calls["n"]

    assert cli.main(["--role", "vm", "get", "vm-open", "--json"]) == 0
    open_out = json.loads(capsys.readouterr().out)
    assert open_out["id"] == "vm-open"

    rc = cli.main(["--role", "vm", "get", "vm-secret", "--json"])
    secret_out = json.loads(capsys.readouterr().out)
    assert rc == 2  # withheld_by_egress_filter — MNPI never reaches the VM caller
    assert secret_out["error"] == "withheld_by_egress_filter"

    assert key_calls["n"] == key_calls_before_vm_read  # VM reads never touched signing

    # the underlying VM snapshot index really is read-only
    vm_read = BrainCore(vault=v, index=_vm_index_on_snapshot(snap_db), role="vm")
    with pytest.raises(sqlite3.OperationalError):
        vm_read.index.conn.execute("INSERT INTO meta(k, v) VALUES ('x', 'y')")
    with pytest.raises(RoleError):
        vm_read.write_note("brain/resources/x.md", "x")
    assert key_calls["n"] == key_calls_before_vm_read  # still never resolved on VM
