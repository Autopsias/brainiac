...
def test_grep_withholds_restricted(built, capsys):
    ...
    # M-1 gates --regex on the bounded `regex` engine being installed; this
    # pattern needs no regex features, so exercise it in literal mode (works
    # identically on a minimal build with no `regex` package).
