...
from cos_s02b_assert_checks import (  # noqa: E402
    archive_eligibility_check, cap_honoured_check, capability_set_check,
    four_chips_check, plan_identity_check, unread_shield_check,
    validator_branch_check, widening_shipped_check)
...
def assertions(res: Result, ctx: dict[str, Any]) -> dict[str, Any]:
    ...
    # Each row's condition logic and detail text live in
    # cos_s02b_assert_checks, one function per assertion, unchanged.
    ok, detail = four_chips_check(chips, FOUR_CHIPS)
    res.add("assertion", "1-four-chips-and-the-matrix", ok, detail)
    ok, detail = archive_eligibility_check(archives)
    res.add("assertion", "2a-archive-eligibility", ok, detail)
    ok, detail = widening_shipped_check(archives, ctx["inputs"]["truth_table"],
                                        READ_NOISE_SIGNAL)
    res.add("assertion", "2b-widening-shipped", ok, detail)
    ok, detail = validator_branch_check(ledger, READ_NOISE_SIGNAL)
    res.add("assertion", "2c-validator-branch-reads-a-produced-field", ok, detail)
    ok, detail = unread_shield_check(run, ledger)
    res.add("assertion", "3-unread-shield", ok, detail)
    ...
    ok, detail = capability_set_check(ctx["inputs"]["run_manifest"] or {},
                                      ctx["inputs"]["handoff"])
    res.add("assertion", "6-run-bound-capability-set", ok, detail)

    # the blast cap — count the APPLIED archives against what the owner set
    ok, detail = cap_honoured_check(ctx["inputs"]["handoff"], len(archives))
    res.add("assertion", "cap-honoured", ok, detail)
    ok, detail = plan_identity_check(ctx["inputs"]["plan"] or {},
                                     ctx["inputs"]["dry_run"] or {},
                                     ctx.get("binding") or {})
    res.add("assertion", "cap-plan-identity", ok, detail)
