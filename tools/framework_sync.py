...
from package_clients import (  # noqa: E402  (reuse the mapping, don't duplicate it)
    BRAINIAC_SKILLS, KERNEL_SKILLS, EXTRAS_SKILLS,
...
def _hash_tree(root: Path) -> dict[str, str]:
    ...
    Generated bytecode (``__pycache__`` / ``*.pyc`` / ``*.pyo``) and Finder
    droppings (``.DS_Store``) are SKIPPED: they are interpreter/OS output,
    not skill content. Field failure 2026-08-15: a test that imported a
    skill's bundled script made Python drop a ``__pycache__/*.pyc`` into the
    CANONICAL ``.claude/skills/<name>/`` tree only; a later audit in the same
    suite run counted it as drift against the mirrors and failed
    ``test_real_repo_tree_is_currently_clean`` — which aborted a release
    publish inside the tag worktree on a repo that was perfectly clean."""
    out: dict[str, str] = {}
