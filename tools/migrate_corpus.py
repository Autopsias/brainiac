...
Acceptance thresholds (r2-codex pass gate, checked by report_coverage):
  ...
  * uncertain labels are QUARANTINED -> treated most-restrictive (MNPI).
  * ZERO known Restricted/MNPI FALSE-NEGATIVES -> verified by the separate
...
human review of every Restricted/MNPI assignment are downstream (corpus-migration
...
MNPI_KW = re.compile(
    r"\b(MNPI|material non[- ]public|insider information|inside information)\b", re.I)
RESTRICTED_KW = re.compile(
    r"\b(Northwind|Contoso|Ventura|Fabrikam|merger|acquisition|valuation|deal terms?|"
    r"counterpart(?:y|ies)|due diligence|term ?sheet|Meridian|separation|"
    r"negotiation|SPA|equity stake|"
    # deal-separation synonyms surfaced by the human spot-review (these were
    # the false-negative class: a Meridian note that did not literally say
    # 'Meridian' in the body). Fail-closed: escalate.
    r"RetainedCo|technology separation|technology segregation|segregation strateg|"
    r"service agreement|transition services|divestiture|spin[- ]?off|day[- ]?1 readiness|"
    r"org-transition|M&A|pre[- ]?signing|not[- ]to[- ]exceed|\bPhase[- ]?0\b|"
    r"\bDay[- ]?1\b)\b", re.I)
...
def classify(zone: str, scan_text: str, body: str, has_fm: bool):
    ...
    quarantine (MNPI) anything genuinely undecidable.

    ``scan_text`` is the FULL note text (frontmatter + body): the S03 spot-review
    found that sensitivity often lives in frontmatter (``projects: [[Meridian]]``,
    ``topics: [Meridian]``) — scanning only the body produced false-negatives."""
    rule = ZONE_RULES.get(zone)
    if rule is None:
        # Unknown zone -> quarantine, most-restrictive, low confidence.
        return "MNPI", "low", f"unknown source zone '{zone}' -> quarantine (default-deny)"
    ...
    if MNPI_KW.search(scan_text):
        if RANK["MNPI"] > RANK[tier]:
            tier, why = "MNPI", why + " + MNPI keyword"
        conf = "medium"
    ...
    if not has_fm and len(body.strip()) < 40 and base_conf == "low":
        return "MNPI", "low", "no frontmatter + negligible body -> quarantine"
...
def iter_md(root: Path):
    ...
    ``.pytest_cache/``, ``_archive/``, ``_owner_vault_scheduled_tasks_staging/``,
...
def main() -> int:
    ...
    for i, p in enumerate(iter_md(src)):
        ...
        is_quarantine = (conf == "low" and tier == "MNPI")
        if is_quarantine:
            quarantined += 1
